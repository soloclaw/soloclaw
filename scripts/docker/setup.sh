...
EXTRA_MOUNTS="${SOLOCLAW_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${SOLOCLAW_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${SOLOCLAW_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${SOLOCLAW_DOCKER_SOCKET:-}"
TIMEZONE="${SOLOCLAW_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$SOLOCLAW_CONFIG_DIR/soloclaw.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == SOLOCLAW_GATEWAY_TOKEN=* ]]; then
      token="${line#SOLOCLAW_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${SOLOCLAW_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$SOLOCLAW_GATEWAY_PORT" "$SOLOCLAW_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$SOLOCLAW_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$SOLOCLAW_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "SOLOCLAW_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "SOLOCLAW_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
SOLOCLAW_CONFIG_DIR="${SOLOCLAW_CONFIG_DIR:-$HOME/.soloclaw}"
SOLOCLAW_WORKSPACE_DIR="${SOLOCLAW_WORKSPACE_DIR:-$HOME/.soloclaw/workspace}"

validate_mount_path_value "SOLOCLAW_CONFIG_DIR" "$SOLOCLAW_CONFIG_DIR"
validate_mount_path_value "SOLOCLAW_WORKSPACE_DIR" "$SOLOCLAW_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "SOLOCLAW_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "SOLOCLAW_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "SOLOCLAW_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "SOLOCLAW_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "SOLOCLAW_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "SOLOCLAW_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$SOLOCLAW_CONFIG_DIR"
mkdir -p "$SOLOCLAW_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$SOLOCLAW_CONFIG_DIR/identity"
mkdir -p "$SOLOCLAW_CONFIG_DIR/agents/main/agent"
mkdir -p "$SOLOCLAW_CONFIG_DIR/agents/main/sessions"

export SOLOCLAW_CONFIG_DIR
export SOLOCLAW_WORKSPACE_DIR
export SOLOCLAW_GATEWAY_PORT="${SOLOCLAW_GATEWAY_PORT:-18789}"
export SOLOCLAW_BRIDGE_PORT="${SOLOCLAW_BRIDGE_PORT:-18790}"
export SOLOCLAW_GATEWAY_BIND="${SOLOCLAW_GATEWAY_BIND:-lan}"
export SOLOCLAW_IMAGE="$IMAGE_NAME"
export SOLOCLAW_DOCKER_APT_PACKAGES="${SOLOCLAW_DOCKER_APT_PACKAGES:-}"
export SOLOCLAW_EXTENSIONS="${SOLOCLAW_EXTENSIONS:-}"
export SOLOCLAW_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export SOLOCLAW_HOME_VOLUME="$HOME_VOLUME_NAME"
export SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS="${SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS:-}"
export SOLOCLAW_SANDBOX="$SANDBOX_ENABLED"
export SOLOCLAW_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export SOLOCLAW_TZ="$TIMEZONE"
...
if [[ -z "${SOLOCLAW_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    SOLOCLAW_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $SOLOCLAW_CONFIG_DIR/soloclaw.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      SOLOCLAW_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      SOLOCLAW_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      SOLOCLAW_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${SOLOCLAW_CONFIG_DIR}:/home/node/.soloclaw"
    gateway_workspace_mount="${SOLOCLAW_WORKSPACE_DIR}:/home/node/.soloclaw/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${SOLOCLAW_INSTALL_DOCKER_CLI:-}" ]]; then
    export SOLOCLAW_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  SOLOCLAW_CONFIG_DIR \
  SOLOCLAW_WORKSPACE_DIR \
  SOLOCLAW_GATEWAY_PORT \
  SOLOCLAW_BRIDGE_PORT \
  SOLOCLAW_GATEWAY_BIND \
  SOLOCLAW_GATEWAY_TOKEN \
  SOLOCLAW_IMAGE \
  SOLOCLAW_EXTRA_MOUNTS \
  SOLOCLAW_HOME_VOLUME \
  SOLOCLAW_DOCKER_APT_PACKAGES \
  SOLOCLAW_EXTENSIONS \
  SOLOCLAW_SANDBOX \
  SOLOCLAW_DOCKER_SOCKET \
  DOCKER_GID \
  SOLOCLAW_INSTALL_DOCKER_CLI \
...
if [[ "$IMAGE_NAME" == "openclaw:local" ]]; then
  ...
  run_docker_build \
    --build-arg "SOLOCLAW_DOCKER_APT_PACKAGES=${SOLOCLAW_DOCKER_APT_PACKAGES}" \
    --build-arg "SOLOCLAW_EXTENSIONS=${SOLOCLAW_EXTENSIONS}" \
    --build-arg "SOLOCLAW_INSTALL_DOCKER_CLI=${SOLOCLAW_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
run_prestart_gateway --user root --entrypoint sh openclaw-gateway -c \
  'find /home/node/.soloclaw -xdev -exec chown node:node {} +; \
   [ -d /home/node/.soloclaw/workspace/.soloclaw ] && chown -R node:node /home/node/.soloclaw/workspace/.soloclaw || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from SOLOCLAW_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $SOLOCLAW_GATEWAY_BIND"
echo "Gateway token: $SOLOCLAW_GATEWAY_TOKEN"
...
echo "Docs: https://docs.soloclaw.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d openclaw-gateway

# --- Sandbox setup (opt-in via SOLOCLAW_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker openclaw-gateway --version >/dev/null 2>&1; then
    ...
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg SOLOCLAW_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (SOLOCLAW_IMAGE=openclaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: SOLOCLAW_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.soloclaw.ai/gateway/sandboxing"
...
echo "Config: $SOLOCLAW_CONFIG_DIR"
echo "Workspace: $SOLOCLAW_WORKSPACE_DIR"
echo "Token: $SOLOCLAW_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f openclaw-gateway"
echo "  ${COMPOSE_HINT} exec openclaw-gateway node dist/index.js health --token \"$SOLOCLAW_GATEWAY_TOKEN\""
