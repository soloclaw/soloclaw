...
MODELS_MODE="${SOLOCLAW_E2E_MODELS:-both}" # both|openai|anthropic
INSTALL_TAG="${SOLOCLAW_INSTALL_TAG:-latest}"
E2E_PREVIOUS_VERSION="${SOLOCLAW_INSTALL_E2E_PREVIOUS:-}"
SKIP_PREVIOUS="${SOLOCLAW_INSTALL_E2E_SKIP_PREVIOUS:-0}"
...
if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: SOLOCLAW_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: SOLOCLAW_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: SOLOCLAW_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: SOLOCLAW_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: SOLOCLAW_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SOLOCLAW_INSTALL_E2E_SKIP_PREVIOUS=1)"
else
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  SOLOCLAW_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  SOLOCLAW_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
...
run_profile() {
  ...
  SESSION_JSONL="$HOME/.soloclaw-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "$HOME/.soloclaw-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
