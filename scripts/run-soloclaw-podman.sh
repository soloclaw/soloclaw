#!/usr/bin/env bash
# Rootless SoloClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-soloclaw-podman.sh launch        # Start gateway
#   ./scripts/run-soloclaw-podman.sh launch setup  # Onboarding wizard
#
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-soloclaw-podman.sh as your normal user so Podman stays rootless."
fi
