...
if openclaw_live_truthy "${SOLOCLAW_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  SOLOCLAW_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${SOLOCLAW_LIVE_PROVIDERS:-}" || -n "${SOLOCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      openclaw_live_collect_auth_dirs_from_csv "${SOLOCLAW_LIVE_PROVIDERS:-}"
      openclaw_live_collect_auth_dirs_from_csv "${SOLOCLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      openclaw_live_collect_auth_files_from_csv "${SOLOCLAW_LIVE_PROVIDERS:-}"
      openclaw_live_collect_auth_files_from_csv "${SOLOCLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${SOLOCLAW_DOCKER_PROFILE_ENV_ONLY:-0}"
...
docker run --rm -t \
  ...
  -e SOLOCLAW_LIVE_MODELS="${SOLOCLAW_LIVE_MODELS:-modern}" \
  -e SOLOCLAW_LIVE_PROVIDERS="${SOLOCLAW_LIVE_PROVIDERS:-}" \
  -e SOLOCLAW_LIVE_MAX_MODELS="${SOLOCLAW_LIVE_MAX_MODELS:-12}" \
  -e SOLOCLAW_LIVE_MODEL_TIMEOUT_MS="${SOLOCLAW_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e SOLOCLAW_LIVE_REQUIRE_PROFILE_KEYS="${SOLOCLAW_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e SOLOCLAW_LIVE_GATEWAY_MODELS="${SOLOCLAW_LIVE_GATEWAY_MODELS:-}" \
  -e SOLOCLAW_LIVE_GATEWAY_PROVIDERS="${SOLOCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e SOLOCLAW_LIVE_GATEWAY_MAX_MODELS="${SOLOCLAW_LIVE_GATEWAY_MAX_MODELS:-}" \
  "${DOCKER_HOME_MOUNT[@]}" \
