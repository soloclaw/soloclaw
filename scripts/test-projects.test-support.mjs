...
const INCLUDE_FILE_ENV_KEY = "SOLOCLAW_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "SOLOCLAW_VITEST_FS_MODULE_CACHE_PATH";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = Number.parseInt(process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL ?? "", 10);
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.SOLOCLAW_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.SOLOCLAW_VITEST_MAX_WORKERS ?? env.SOLOCLAW_TEST_WORKERS,
  );
...
export function resolveParallelFullSuiteConcurrency(specCount, env = process.env, hostInfo) {
  const override = parsePositiveInt(env.SOLOCLAW_TEST_PROJECTS_PARALLEL);
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.SOLOCLAW_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.SOLOCLAW_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
