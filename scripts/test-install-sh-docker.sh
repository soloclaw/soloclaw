...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${SOLOCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$SOLOCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.SOLOCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `SOLOCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${SOLOCLAW_INSTALL_SMOKE_IMAGE:-openclaw-install-smoke:local}"
NONROOT_IMAGE="${SOLOCLAW_INSTALL_NONROOT_IMAGE:-openclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${SOLOCLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${SOLOCLAW_INSTALL_URL:-https://openclaw.bot/install.sh}"
CLI_INSTALL_URL="${SOLOCLAW_INSTALL_CLI_URL:-https://openclaw.bot/install-cli.sh}"
PACKAGE_NAME="${SOLOCLAW_INSTALL_PACKAGE:-openclaw}"
SKIP_NONROOT="${SOLOCLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${SOLOCLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${SOLOCLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${SOLOCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${SOLOCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${SOLOCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${SOLOCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${SOLOCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${SOLOCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${SOLOCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${SOLOCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${SOLOCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (SOLOCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e SOLOCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SOLOCLAW_INSTALL_METHOD=npm \
    -e SOLOCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e SOLOCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e SOLOCLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e SOLOCLAW_NO_ONBOARD=1 \
    -e SOLOCLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e SOLOCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SOLOCLAW_INSTALL_SMOKE_MODE=update \
    -e SOLOCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e SOLOCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e SOLOCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e SOLOCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e SOLOCLAW_NO_ONBOARD=1 \
    -e SOLOCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (SOLOCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${UPDATE_DOCKER_HOST_ARGS[@]}" \
      -e SOLOCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e SOLOCLAW_INSTALL_SMOKE_MODE=npm-global \
      -e SOLOCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e SOLOCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e SOLOCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e SOLOCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (SOLOCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e SOLOCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SOLOCLAW_INSTALL_METHOD=npm \
    -e SOLOCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e SOLOCLAW_NO_ONBOARD=1 \
    -e SOLOCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${SOLOCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (SOLOCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e SOLOCLAW_INSTALL_URL="$INSTALL_URL" \
  -e SOLOCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e SOLOCLAW_NO_ONBOARD=1 \
  -e SOLOCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
