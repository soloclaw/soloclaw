...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./soloclaw-npm-release-check.ts";
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  ...
  try {
    ...
    console.log(`soloclaw-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error(
      "Usage: node --import tsx scripts/soloclaw-npm-postpublish-verify.ts <version>",
    );
  }
  ...
  console.log(
    `soloclaw-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`soloclaw-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
