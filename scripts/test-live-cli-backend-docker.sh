...
DEFAULT_PROVIDER="${SOLOCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${SOLOCLAW_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${SOLOCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${SOLOCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
TEMP_DIRS=()
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: SOLOCLAW_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: SOLOCLAW_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi

if [[ "$CLI_AUTH_MODE" == "api-key" && "$CLI_PROVIDER" == "codex-cli" ]]; then
  if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    echo "ERROR: SOLOCLAW_LIVE_CLI_BACKEND_AUTH=api-key for codex-cli requires OPENAI_API_KEY." >&2
    exit 1
  fi
fi
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export SOLOCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export SOLOCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export SOLOCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
elif [[ -n "${SOLOCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
...
provider="${SOLOCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${SOLOCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${SOLOCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${SOLOCLAW_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export SOLOCLAW_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
if [ -n "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  npm install -g "$docker_package"
fi
if [ "$provider" = "codex-cli" ] && [ "${SOLOCLAW_LIVE_CLI_BACKEND_AUTH:-auto}" = "api-key" ]; then
  codex_login_command="${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND:-$NPM_CONFIG_PREFIX/bin/codex}"
...
if [ -n "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND}"
  "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND}" -V || "${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${SOLOCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${SOLOCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${SOLOCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${SOLOCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${SOLOCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
...
cd "$tmp_dir"
if [ "${SOLOCLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG:-0}" = "1" ]; then
  node --import tsx /src/scripts/prepare-codex-ci-config.ts "$HOME/.codex/config.toml" "$tmp_dir"
fi
pnpm test:live src/gateway/gateway-cli-backend.live.test.ts
EOF

if [[ "${SOLOCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (SOLOCLAW_SKIP_DOCKER_BUILD=1)"
else
...
DOCKER_AUTH_ENV=(
  -e SOLOCLAW_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "codex-cli" && "$CLI_AUTH_MODE" == "api-key" ]]; then
  ...
  DOCKER_AUTH_ENV+=(
    ...
    -e SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="$SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e SOLOCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e SOLOCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="${SOLOCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi
...
docker run --rm -t \
  ...
  -e SOLOCLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG="${SOLOCLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG:-0}" \
  -e SOLOCLAW_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e SOLOCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e SOLOCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e SOLOCLAW_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e SOLOCLAW_LIVE_TEST=1 \
  -e SOLOCLAW_LIVE_CLI_BACKEND=1 \
  -e SOLOCLAW_LIVE_CLI_BACKEND_DEBUG="${SOLOCLAW_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e SOLOCLAW_CLI_BACKEND_LOG_OUTPUT="${SOLOCLAW_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_COMMAND="${SOLOCLAW_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_ARGS="${SOLOCLAW_LIVE_CLI_BACKEND_ARGS:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_RESUME_ARGS="${SOLOCLAW_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_CLEAR_ENV="${SOLOCLAW_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${SOLOCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="${SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="${SOLOCLAW_LIVE_CLI_BACKEND_IMAGE_MODE:-}" \
  "${DOCKER_HOME_MOUNT[@]}" \
  "${DOCKER_EXTRA_ENV_FILES[@]}" \
