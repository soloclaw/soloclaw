#!/bin/bash
# Setup SoloClaw Auth Management System
...
echo "=== SoloClaw Auth System Setup ==="
echo ""
...
# Phone notification
echo ""
echo "  SoloClaw message: Send warning via SoloClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/soloclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/soloclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/soloclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now soloclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status soloclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
