...
#   NOTIFY_PHONE - Phone number to send SoloClaw notification (e.g., +1234567890)
...
STATE_FILE="$HOME/.soloclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SoloClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        ...
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SoloClaw to $NOTIFY_PHONE..."
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SoloClaw is down. Run: ssh l36 '~/openclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
