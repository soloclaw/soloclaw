...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[soloclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[soloclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[soloclaw] prefer: corepack pnpm install",
  ].join("\n");
}
