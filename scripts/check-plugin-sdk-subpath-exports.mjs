...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("soloclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced soloclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced soloclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
