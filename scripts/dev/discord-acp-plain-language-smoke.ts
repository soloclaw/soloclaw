...
function resolveStateDir(): string {
  const override = process.env.SOLOCLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.SOLOCLAW_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "  SOLOCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  SOLOCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  SOLOCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  SOLOCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  SOLOCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  SOLOCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  SOLOCLAW_DISCORD_SMOKE_AGENT\n" +
    "  SOLOCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  SOLOCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  SOLOCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  SOLOCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  SOLOCLAW_DISCORD_SMOKE_SOLOCLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.SOLOCLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.SOLOCLAW_DISCORD_SMOKE_DRIVER || "token";
    ...
    normalizedDriverMode === "webhook"
      ...
      : normalizedDriverMode === "soloclaw"
        ...
        : normalizedDriverMode === "token"
          ...
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.SOLOCLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.SOLOCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.SOLOCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.SOLOCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.SOLOCLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.SOLOCLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.SOLOCLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.SOLOCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.SOLOCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
...
async function openclawCliJson<T>(params: { openclawBin: string; args: string[] }): Promise<T> {
  ...
  if (!stdout) {
    throw new Error(`soloclaw ${params.args.join(" ")} returned empty stdout`);
  }
...
async function run(): Promise<SuccessResult | FailureResult> {
  ...
  try {
    if (args.driverMode === "token") {
      ...
      if (!sentMessageId) {
        throw new Error("soloclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from SoloClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
