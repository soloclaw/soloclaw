...
export function isLocalCheckEnabled(env) {
  const raw = env.SOLOCLAW_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (defaultProjectRun) {
    ...
    insertBeforeSeparator(
      nextArgs,
      "--tsBuildInfoFile",
      nextEnv.SOLOCLAW_TSGO_BUILD_INFO_FILE ?? DEFAULT_LOCAL_TSGO_BUILD_INFO_FILE,
    );
  }
  ...
  if (nextEnv.SOLOCLAW_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.SOLOCLAW_TSGO_PPROF_DIR);
  }
...
export function shouldAcquireLocalHeavyCheckLockForOxlint(
  ...
  if (env.SOLOCLAW_OXLINT_FORCE_LOCK === "1") {
    return true;
  }
...
export function shouldAcquireLocalHeavyCheckLockForTsgo(args, env = process.env) {
  if (env.SOLOCLAW_TSGO_FORCE_LOCK === "1") {
    return true;
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const timeoutMs = readPositiveInt(
    env.SOLOCLAW_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
  );
  const pollMs = readPositiveInt(env.SOLOCLAW_HEAVY_CHECK_LOCK_POLL_MS, DEFAULT_LOCK_POLL_MS);
  const progressMs = readPositiveInt(
    env.SOLOCLAW_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
  );
  const staleLockMs = readPositiveInt(
    env.SOLOCLAW_HEAVY_CHECK_STALE_LOCK_MS,
...
function readLocalCheckMode(env) {
  const raw = env.SOLOCLAW_LOCAL_CHECK_MODE?.trim().toLowerCase();
  ...
  // into full-speed runs explicitly with SOLOCLAW_LOCAL_CHECK_MODE=full.
  return "throttled";
}
