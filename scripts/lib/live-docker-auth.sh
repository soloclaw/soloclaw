...
SOLOCLAW_DOCKER_LIVE_AUTH_ALL=(.gemini .minimax)
SOLOCLAW_DOCKER_LIVE_AUTH_FILES_ALL=(
  .codex/auth.json
  .codex/config.toml
  .claude.json
...
openclaw_live_collect_auth_dirs_from_override() {
  ...
  raw="$(openclaw_live_trim "${SOLOCLAW_DOCKER_AUTH_DIRS:-}")"
  [[ -n "$raw" ]] || return 1
  case "$raw" in
    all)
      printf '%s\n' "${SOLOCLAW_DOCKER_LIVE_AUTH_ALL[@]}"
      return 0
      ;;
    none)
      return 0
