...
function isSystemThrottleDisabled(env) {
  const normalized = env.SOLOCLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.SOLOCLAW_VITEST_MAX_WORKERS ?? env.SOLOCLAW_TEST_WORKERS);
  if (override !== null) {
