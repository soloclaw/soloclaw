...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.soloclaw) ? packageJson.soloclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.soloclaw?.channel?.id) || left.name;
    const rightId = trimString(right.soloclaw?.channel?.id) || right.name;
