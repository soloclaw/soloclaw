...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeTempDir(pluginDir, `.soloclaw-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.soloclaw-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeTempDir(
    targetParentDir,
    `.soloclaw-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".soloclaw-runtime-deps-stamp.json");
}
