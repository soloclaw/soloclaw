...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.SOLOCLAW_VITEST_MAX_WORKERS || env.SOLOCLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      SOLOCLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL &&
        !process.env.SOLOCLAW_VITEST_MAX_WORKERS &&
        !process.env.SOLOCLAW_TEST_WORKERS &&
