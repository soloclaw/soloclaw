...
SOLOCLAW_WORKSPACE_DIR="${SOLOCLAW_WORKSPACE_DIR:-}"
SOLOCLAW_IMAGE="${SOLOCLAW_PODMAN_IMAGE:-${SOLOCLAW_IMAGE:-openclaw:local}}"
SOLOCLAW_CONTAINER_NAME="${SOLOCLAW_PODMAN_CONTAINER:-openclaw}"
...
if [[ -n "${SOLOCLAW_PODMAN_QUADLET:-}" ]]; then
  case "${SOLOCLAW_PODMAN_QUADLET,,}" in
...
if [[ "$SOLOCLAW_IMAGE" == "openclaw:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set SOLOCLAW_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$SOLOCLAW_HOME" ]]; then
  SOLOCLAW_HOME="$(resolve_user_home "$SOLOCLAW_USER")"
fi
if [[ -z "$SOLOCLAW_HOME" ]]; then
  echo "Unable to resolve HOME for user $SOLOCLAW_USER." >&2
  exit 1
fi
if [[ -z "$SOLOCLAW_CONFIG_DIR" ]]; then
  SOLOCLAW_CONFIG_DIR="$SOLOCLAW_HOME/.soloclaw"
fi
if [[ -z "$SOLOCLAW_WORKSPACE_DIR" ]]; then
  SOLOCLAW_WORKSPACE_DIR="$SOLOCLAW_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$SOLOCLAW_HOME"
validate_mount_source_path "config directory" "$SOLOCLAW_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$SOLOCLAW_WORKSPACE_DIR"
validate_container_name "$SOLOCLAW_CONTAINER_NAME"
validate_image_name "$SOLOCLAW_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$SOLOCLAW_CONFIG_DIR" "$SOLOCLAW_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$SOLOCLAW_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$SOLOCLAW_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${SOLOCLAW_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "SOLOCLAW_DOCKER_APT_PACKAGES=${SOLOCLAW_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${SOLOCLAW_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "SOLOCLAW_EXTENSIONS=${SOLOCLAW_EXTENSIONS}")
fi

if [[ "$SOLOCLAW_IMAGE" == "openclaw:local" ]]; then
  echo "Building image $SOLOCLAW_IMAGE ..."
  podman build -t "$SOLOCLAW_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$SOLOCLAW_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $SOLOCLAW_IMAGE"
  else
    echo "Pulling image $SOLOCLAW_IMAGE ..."
    podman pull "$SOLOCLAW_IMAGE"
  fi
fi

ENV_FILE="$SOLOCLAW_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated SOLOCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "SOLOCLAW_PODMAN_CONTAINER" "$SOLOCLAW_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "SOLOCLAW_PODMAN_IMAGE" "$SOLOCLAW_IMAGE"

CONFIG_JSON="$SOLOCLAW_CONFIG_DIR/soloclaw.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  SOLOCLAW_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$SOLOCLAW_HOME")"
  SOLOCLAW_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$SOLOCLAW_CONFIG_DIR")"
  SOLOCLAW_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$SOLOCLAW_WORKSPACE_DIR")"
  SOLOCLAW_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$SOLOCLAW_IMAGE")"
  SOLOCLAW_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$SOLOCLAW_CONTAINER_NAME")"
  sed \
    -e "s|{{SOLOCLAW_HOME}}|$SOLOCLAW_HOME_ESCAPED|g" \
    -e "s|{{SOLOCLAW_CONFIG_DIR}}|$SOLOCLAW_CONFIG_ESCAPED|g" \
    -e "s|{{SOLOCLAW_WORKSPACE_DIR}}|$SOLOCLAW_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$SOLOCLAW_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$SOLOCLAW_CONTAINER_ESCAPED|g" \
