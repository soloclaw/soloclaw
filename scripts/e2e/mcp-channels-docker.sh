...
IMAGE_NAME="${SOLOCLAW_IMAGE:-openclaw-mcp-channels-e2e}"
PORT="18789"
...
docker run --rm \
  ...
  -e "SOLOCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "SOLOCLAW_SKIP_CRON=1" \
  -e "SOLOCLAW_SKIP_CANVAS_HOST=1" \
  -e "SOLOCLAW_STATE_DIR=/tmp/openclaw-state" \
  -e "SOLOCLAW_CONFIG_PATH=/tmp/openclaw-state/soloclaw.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
