...
if [ -f dist/index.mjs ]; then
  SOLOCLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  SOLOCLAW_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export SOLOCLAW_ENTRY
...
SOLOCLAW_PLUGIN_HOME="$HOME/.soloclaw/$BUNDLED_PLUGIN_ROOT_DIR"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$SOLOCLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$SOLOCLAW_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$SOLOCLAW_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
demo_plugin_root="$SOLOCLAW_PLUGIN_HOME/$demo_plugin_id"
...
node "$SOLOCLAW_ENTRY" plugins list --json > /tmp/plugins.json
node "$SOLOCLAW_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
run_logged install-tgz node "$SOLOCLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$SOLOCLAW_ENTRY" plugins list --json > /tmp/plugins2.json
node "$SOLOCLAW_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
run_logged install-dir node "$SOLOCLAW_ENTRY" plugins install "$dir_plugin"
node "$SOLOCLAW_ENTRY" plugins list --json > /tmp/plugins3.json
node "$SOLOCLAW_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
run_logged install-file node "$SOLOCLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$SOLOCLAW_ENTRY" plugins list --json > /tmp/plugins4.json
node "$SOLOCLAW_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$SOLOCLAW_PLUGIN_HOME/$bundle_plugin_id"
...
node "$SOLOCLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
run_logged install-marketplace-shortcut node "$SOLOCLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
run_logged install-marketplace-direct node "$SOLOCLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$SOLOCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$SOLOCLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$SOLOCLAW_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
run_logged update-marketplace-shortcut-dry-run node "$SOLOCLAW_ENTRY" plugins update marketplace-shortcut --dry-run
run_logged update-marketplace-shortcut node "$SOLOCLAW_ENTRY" plugins update marketplace-shortcut
node "$SOLOCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$SOLOCLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
