...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$SOLOCLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$SOLOCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }
  ...
  set_isolated_openclaw_env() {
    ...
    export SOLOCLAW_HOME="$home_dir"
    export SOLOCLAW_STATE_DIR="$home_dir/.soloclaw"
    export SOLOCLAW_CONFIG_PATH="$SOLOCLAW_STATE_DIR/soloclaw.json"
    mkdir -p "$SOLOCLAW_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    set_isolated_openclaw_env "$home_dir"
    run_case_logged local-basic node "$SOLOCLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$SOLOCLAW_STATE_DIR/workspace"
    config_path="$SOLOCLAW_CONFIG_PATH"
    sessions_dir="$SOLOCLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_openclaw_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$SOLOCLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$SOLOCLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$SOLOCLAW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$SOLOCLAW_ENTRY\" configure --section skills" send_skills_flow
