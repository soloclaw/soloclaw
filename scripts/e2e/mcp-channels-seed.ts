...
async function main() {
  const stateDir = process.env.SOLOCLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".soloclaw");
  const configPath =
    process.env.SOLOCLAW_CONFIG_PATH?.trim() || path.join(stateDir, "soloclaw.json");
