...
spawn {*}$cmd
send -- "printf '__SOLOCLAW_READY__\\n'\r"
expect "__SOLOCLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/openclaw-prl.sh <<'__SOLOCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__SOLOCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/openclaw-prl.sh; rc=\$?; rm -f /tmp/openclaw-prl.sh; printf '__SOLOCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
set saw_rc 0
expect {
  -re {__SOLOCLAW_RC__:(-?[0-9]+)} {
...
markers = [
    ...
    if line.startswith("__SOLOCLAW_RC__:")
]
if not markers:
    raise SystemExit(1)
...
printf '__SOLOCLAW_RC__:%s\n' "\$status"
printf '%s\n' "\$status" > "$done_path"
exit "\$status"
EOF
)"
  write_runner_cmd="/bin/rm -f $(shell_quote "$runner_path")"$'\n'
  write_runner_cmd+="cat > $(shell_quote "$runner_path") <<'__SOLOCLAW_RUNNER__'"$'\n'
  write_runner_cmd+="$runner_body"$'\n'
  write_runner_cmd+="__SOLOCLAW_RUNNER__"$'\n'
...
matches = re.findall(r"^__SOLOCLAW_RC__:(-?\d+)$", text, flags=re.MULTILINE)
if not matches:
    raise SystemExit(1)
print(matches[-1])
PY
...
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 \
  $GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY update --channel dev --yes --json
EOF
  ...
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SOLOCLAW_ENTRY" --version
  printf 'update-dev: git-status\n'
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SOLOCLAW_ENTRY" update status --json
}

verify_dev_channel_update() {
  local status_json
  status_json="$(guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SOLOCLAW_ENTRY" update status --json)"
  printf '%s\n' "$status_json"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_SOLOCLAW_BIN" --version 2>&1
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
/usr/bin/env \\
  ...
  SOLOCLAW_HOME=$(shell_quote "$guest_home") \\
  SOLOCLAW_STATE_DIR=$(shell_quote "$guest_home/.soloclaw") \\
  SOLOCLAW_CONFIG_PATH=$(shell_quote "$guest_home/.soloclaw/soloclaw.json") \\
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_SOLOCLAW_ENTRY") gateway run --bind loopback --port 18789 --force \\
...
verify_gateway() {
  ...
  for attempt in 1 2 3 4; do
    if guest_current_user_exec "$GUEST_SOLOCLAW_BIN" gateway status --deep --require-rpc --timeout 5000; then
      return 0
    fi
    if (( attempt < 4 )); then
...
show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_SOLOCLAW_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_SOLOCLAW_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_SOLOCLAW_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SOLOCLAW_ENTRY" models set "$MODEL_ID"
  guest_current_user_sh "$(cat <<EOF
export PATH=$(shell_quote "$GUEST_EXEC_PATH")
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") \
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_SOLOCLAW_ENTRY") agent \
  --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_SOLOCLAW_BIN" dashboard --no-open \
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/openclaw-discord-token <<'__SOLOCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__SOLOCLAW_TOKEN__
cat >/tmp/openclaw-discord-guilds.json <<'__SOLOCLAW_GUILDS__'
$guilds_json
__SOLOCLAW_GUILDS__
token="\$(tr -d '\n' </tmp/openclaw-discord-token)"
guilds_json="\$(cat /tmp/openclaw-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_SOLOCLAW_ENTRY channels status --probe --json
