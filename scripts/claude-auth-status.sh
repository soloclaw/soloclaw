...
# Checks both Claude Code and SoloClaw auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
SOLOCLAW_AUTH="$HOME/.soloclaw/agents/main/agent/auth-profiles.json"
...
check_openclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$SOLOCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        openclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$SOLOCLAW_AUTH" 2>/dev/null || echo "0")
    fi
...
echo ""
echo "SoloClaw Auth (~/.soloclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$SOLOCLAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$SOLOCLAW_AUTH" 2>/dev/null || echo "0")
...
if systemctl --user is-active openclaw >/dev/null 2>&1; then
    echo -e "SoloClaw service: ${GREEN}running${NC}"
else
    echo -e "SoloClaw service: ${RED}NOT running${NC}"
fi
