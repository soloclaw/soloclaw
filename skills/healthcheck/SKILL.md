---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SoloClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SoloClaw cron scheduling for periodic checks, or version status checks on a machine running SoloClaw (laptop, workstation, Pi, VPS).
---

# SoloClaw Host Hardening

## Overview

Assess and harden the host running OpenClaw, then align it to a user-defined risk tolerance without breaking access. Use SoloClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SoloClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. SoloClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SoloClaw can access sensitive data.
...
### 2) Run SoloClaw security audits (read-only)

As part of the default read-only checks, run `soloclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `soloclaw security audit` (faster, non-probing)
2. `soloclaw security audit --json` (structured output)

Offer to apply SoloClaw safe defaults (numbered):

1. `soloclaw security audit --fix`

Be explicit that `--fix` only tightens SoloClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SoloClaw version/update status (read-only)

As part of the default read-only checks, run `soloclaw update status`.
...
- Credential hygiene notes (location of SoloClaw creds, prefer disk encryption)
...
- SoloClaw security audit (re-run)
...
After SoloClaw install or first hardening pass, run at least one baseline audit and version check:

- `soloclaw security audit`
- `soloclaw security audit --deep`
- `soloclaw update status`

Ongoing monitoring is recommended. Use the SoloClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `soloclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `soloclaw update status`
...
Before creating, `soloclaw cron list` and match on exact `name`. If found, `soloclaw cron edit <id> ...`.
If not found, `soloclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `soloclaw update status` (preferred for source checkouts and channels)
2. `npm view openclaw version` (published npm version)

## SoloClaw command accuracy

Use only supported commands and flags:

- `soloclaw security audit [--deep] [--fix] [--json]`
- `soloclaw status` / `soloclaw status --deep`
- `soloclaw health --json`
- `soloclaw update status`
- `soloclaw cron add|list|runs|run`

Do not invent CLI flags or imply SoloClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SoloClaw compaction:
