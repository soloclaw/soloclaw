...
In the active OpenClaw config file (`$OPENCLAW_CONFIG_PATH`, default `~/.soloclaw/openclaw.json`):
...
1. Check server bind: `CONFIG_PATH="${OPENCLAW_CONFIG_PATH:-${OPENCLAW_STATE_DIR:-$HOME/.soloclaw}/openclaw.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
