...
description: Investigate `pnpm test` memory growth, Vitest worker OOMs, and suspicious RSS increases in SoloClaw using the `scripts/test-parallel.mjs` heap snapshot tooling. Use when Codex needs to reproduce test-lane memory growth, collect repeated `.heapsnapshot` files, compare snapshots from the same worker PID, triage likely transformed-module retention versus likely runtime leaks, and fix or reduce the impact by patching cleanup logic or isolating hotspot tests.
---

# SoloClaw Test Heap Leaks
