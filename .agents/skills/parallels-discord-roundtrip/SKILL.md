...
```bash
export SOLOCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.soloclaw/soloclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env SOLOCLAW_PARALLELS_DISCORD_TOKEN \
