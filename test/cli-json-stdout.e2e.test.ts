    ...
    await withTempHome(
      async (tempHome) => {
        ...
        const env = {
          ...
          SOLOCLAW_TEST_FAST: "1",
        };
        delete env.SOLOCLAW_HOME;
        delete env.SOLOCLAW_STATE_DIR;
        delete env.SOLOCLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "soloclaw.mjs");
