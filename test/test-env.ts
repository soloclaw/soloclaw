...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.SOLOCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "SOLOCLAW_TEST_FAST", value: process.env.SOLOCLAW_TEST_FAST },
    {
      key: "SOLOCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.SOLOCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "SOLOCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.SOLOCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "SOLOCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.SOLOCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "SOLOCLAW_STATE_DIR", value: process.env.SOLOCLAW_STATE_DIR },
    { key: "SOLOCLAW_CONFIG_PATH", value: process.env.SOLOCLAW_CONFIG_PATH },
    { key: "SOLOCLAW_GATEWAY_PORT", value: process.env.SOLOCLAW_GATEWAY_PORT },
    { key: "SOLOCLAW_BRIDGE_ENABLED", value: process.env.SOLOCLAW_BRIDGE_ENABLED },
    { key: "SOLOCLAW_BRIDGE_HOST", value: process.env.SOLOCLAW_BRIDGE_HOST },
    { key: "SOLOCLAW_BRIDGE_PORT", value: process.env.SOLOCLAW_BRIDGE_PORT },
    { key: "SOLOCLAW_CANVAS_HOST_PORT", value: process.env.SOLOCLAW_CANVAS_HOST_PORT },
    { key: "SOLOCLAW_TEST_HOME", value: process.env.SOLOCLAW_TEST_HOME },
    { key: "SOLOCLAW_AGENT_DIR", value: process.env.SOLOCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  process.env.SOLOCLAW_TEST_HOME = tempHome;
  process.env.SOLOCLAW_TEST_FAST = "1";
  process.env.SOLOCLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.SOLOCLAW_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.SOLOCLAW_BRIDGE_ENABLED;
  delete process.env.SOLOCLAW_BRIDGE_HOST;
  delete process.env.SOLOCLAW_BRIDGE_PORT;
  delete process.env.SOLOCLAW_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.SOLOCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const realConfigPath = params.env.SOLOCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.SOLOCLAW_CONFIG_PATH, params.realHome)
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.SOLOCLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.SOLOCLAW_LIVE_USE_REAL_HOME);
