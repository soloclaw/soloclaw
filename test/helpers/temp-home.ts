...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openclawHome: process.env.SOLOCLAW_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SOLOCLAW_HOME", snapshot.openclawHome);
  restoreKey("SOLOCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SOLOCLAW_HOME.
  delete process.env.SOLOCLAW_HOME;
  process.env.SOLOCLAW_STATE_DIR = path.join(base, ".soloclaw");
...
export async function withTempHome<T>(
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".soloclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
