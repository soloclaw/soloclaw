...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ SOLOCLAW_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ SOLOCLAW_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ SOLOCLAW_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from SOLOCLAW_VITEST_INCLUDE_FILE", () => {
    ...
    try {
      ...
      const config = createScopedVitestConfig(["src/utils/**/*.test.ts"], {
        dir: "src",
        env: {
          SOLOCLAW_VITEST_INCLUDE_FILE: includeFile,
        },
      });
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from SOLOCLAW_VITEST_INCLUDE_FILE", () => {
