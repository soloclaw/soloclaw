...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "soloclaw-launcher-");
  await fs.copyFile(
...
describe("soloclaw launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      ...
      'import "missing-soloclaw-launcher-dep";\nexport {};\n',
      "utf8",
    );
    ...
    expect(result.stderr).toContain("missing-soloclaw-launcher-dep");
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:soloclaw/soloclaw#<ref>");
  });
});
