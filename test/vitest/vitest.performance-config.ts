...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.SOLOCLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.SOLOCLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.SOLOCLAW_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.SOLOCLAW_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.SOLOCLAW_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.SOLOCLAW_VITEST_PRINT_IMPORT_BREAKDOWN)) {
