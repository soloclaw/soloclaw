...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.SOLOCLAW_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.SOLOCLAW_TEST_HOME = testHome;
...
export default class SoloClawNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    ...
    const orderLogPath = process.env.SOLOCLAW_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
