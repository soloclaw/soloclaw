...
} from "../../scripts/soloclaw-cross-os-release-checks.ts";

describe("scripts/soloclaw-cross-os-release-checks", () => {
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        SOLOCLAW_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        SOLOCLAW_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        SOLOCLAW_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "soloclaw-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "soloclaw-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
