  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...process.env,
          SOLOCLAW_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  it("keeps explicit parallel overrides ahead of the host-aware profile", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        61,
        {
          SOLOCLAW_TEST_PROJECTS_PARALLEL: "3",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
  ...
  it("splits untargeted runs into fixed core shards and per-extension configs", () => {
    const previousParallel = process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.SOLOCLAW_TEST_PROJECTS_SERIAL;
    delete process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    delete process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    process.env.SOLOCLAW_TEST_PROJECTS_SERIAL = "1";
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.SOLOCLAW_TEST_PROJECTS_SERIAL;
      } else {
        process.env.SOLOCLAW_TEST_PROJECTS_SERIAL = previousSerial;
      }
    }
  });

  it("expands untargeted local runs to leaf project configs by default", () => {
    const previousLeafShards = process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS;
    const previousParallel = process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.SOLOCLAW_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    const previousVitestMaxWorkers = process.env.SOLOCLAW_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.SOLOCLAW_TEST_WORKERS;
    delete process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.SOLOCLAW_TEST_PROJECTS_SERIAL;
    delete process.env.CI;
    delete process.env.GITHUB_ACTIONS;
    delete process.env.SOLOCLAW_VITEST_MAX_WORKERS;
    delete process.env.SOLOCLAW_TEST_WORKERS;
    try {
      ...
      if (previousLeafShards === undefined) {
        ...
        process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
      }
      if (previousParallel === undefined) {
        delete process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
      } else {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.SOLOCLAW_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.SOLOCLAW_TEST_WORKERS;
      } else {
        process.env.SOLOCLAW_TEST_WORKERS = previousTestWorkers;
      }
    }
  });

  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.SOLOCLAW_TEST_PROJECTS_SERIAL;
    delete process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    process.env.SOLOCLAW_TEST_PROJECTS_SERIAL = "1";
    process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("can expand full-suite shards to project configs for perf experiments", () => {
    const previous = process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS;
    process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS = "1";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.SOLOCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.SOLOCLAW_TEST_PROJECTS_LEAF_SHARDS;
    process.env.SOLOCLAW_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
...
describe("scripts/test-projects parallel cache paths", () => {
  it("assigns isolated Vitest fs-module cache paths per parallel shard", () => {
    ...
    expect(specs.map((spec) => spec.env)).toEqual([
      {
        SOLOCLAW_VITEST_FS_MODULE_CACHE_PATH: path.join(
          "/repo",
          "node_modules",
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { SOLOCLAW_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.SOLOCLAW_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
