...
function readSoloClawBlock(packageJson: unknown) {
...
export function normalizeExternalPluginCompatibility(
  ...
  const { root, compat, build, install } = readSoloClawBlock(packageJson);
  ...
  const builtWithSoloClawVersion = normalizeOptionalString(build?.openclawVersion) ?? version;
  if (builtWithSoloClawVersion) {
    compatibility.builtWithSoloClawVersion = builtWithSoloClawVersion;
  }
...
export function listMissingExternalCodePluginFieldPaths(packageJson: unknown): string[] {
  const { compat, build } = readSoloClawBlock(packageJson);
