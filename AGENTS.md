...
- Repo: https://github.com/soloclaw/soloclaw
...
- Bundled plugin naming: for repo-owned workspace plugins, keep the canonical plugin id aligned across `soloclaw.plugin.json:id`, the default workspace folder name, and package names anchored to the same id (`@soloclaw/<id>` or approved suffix forms like `-provider`, `-plugin`, `-speech`, `-sandbox`, `-media-understanding`). Keep `openclaw.install.npmSpec` equal to the package name and `openclaw.channel.id` equal to the plugin id when present. Exceptions must be explicit and covered by the repo invariant test.
...
- Plugins: install runs `npm install --omit=dev` in plugin dir; runtime deps must live in `dependencies`. Avoid `workspace:*` in `dependencies` (npm install breaks); put `openclaw` in `devDependencies` or `peerDependencies` instead (runtime resolves `soloclaw/plugin-sdk` via jiti alias).
- Import boundaries: extension production code should treat `soloclaw/plugin-sdk/*` plus local `api.ts` / `runtime-api.ts` barrels as the public surface. Do not import core `src/**`, `src/plugin-sdk-internal/**`, or another extension's `src/**` directly.
- Installers served from `https://soloclaw.ai/*`: live in the sibling repo `../soloclaw.ai` (`public/install.sh`, `public/install-cli.sh`, `public/install.ps1`).
...
- Plugin and extension boundary:
  ...
  - Rule: extensions must cross into core only through `soloclaw/plugin-sdk/*`, manifest metadata, and documented runtime helpers. Do not import `src/**` from extension production code.
  ...
  - Rule: for legacy config specifically, prefer doctor-owned repair paths over startup/load-time core migrations. Do not add new plugin-specific legacy migration logic to shared core/runtime surfaces when `soloclaw doctor --fix` can own it.
...
- Config contract boundary:
  ...
  - Do not reintroduce removed legacy aliases into public types/schema/help/baselines “for convenience”. If old configs still need to load, handle that in `legacy.migrations.*`, config ingest, or `soloclaw doctor --fix`.
...
- Config: use `soloclaw config set ...`; ensure `gateway.mode=local` is set.
- Discord: store raw token only (no `DISCORD_BOT_TOKEN=` prefix).
- Restart: stop old gateway and run:
  `pkill -9 -f openclaw-gateway || true; nohup soloclaw gateway run --bind loopback --port 18789 --force > /tmp/openclaw-gateway.log 2>&1 &`
- Verify: `soloclaw channels status --probe`, `ss -ltnp | rg 18789`, `tail -n 120 /tmp/openclaw-gateway.log`.
...
- Run CLI in dev: `pnpm soloclaw ...` (bun) or `pnpm dev`.
- Node remains supported for running built output (`dist/*`) and production installs.
- Mac packaging (dev): `scripts/package-mac-app.sh` defaults to current arch.
- Type-check/build: `pnpm build`
- TypeScript checks: `pnpm tsgo`
- Lint/format: `pnpm check`
- Local agent/dev shells default to host-aware `SOLOCLAW_LOCAL_CHECK=1` behavior for `pnpm tsgo` and `pnpm lint`; set `SOLOCLAW_LOCAL_CHECK_MODE=throttled` to force the lower-memory profile, `SOLOCLAW_LOCAL_CHECK_MODE=full` to keep lock-only behavior, or `SOLOCLAW_LOCAL_CHECK=0` in CI/shared runs.
...
- Extension SDK self-import guardrail: inside an extension package, do not import that same extension via `soloclaw/plugin-sdk/<extension>` from production files. Route internal imports through a local barrel such as `./api.ts` or `./runtime-api.ts`, and keep the `plugin-sdk/<extension>` path as the external contract only.
- Extension package boundary guardrail: inside a bundled plugin package, do not use relative imports/exports that resolve outside that same package root. If shared code belongs in the plugin SDK, import `soloclaw/plugin-sdk/<subpath>` instead of reaching into `src/plugin-sdk/**` or other repo paths via `../`.
- Extension API surface rule: `soloclaw/plugin-sdk/<subpath>` is the only public cross-package contract for extension-facing SDK code. If an extension needs a new seam, add a public subpath first; do not reach into `src/plugin-sdk/**` by relative path.
...
- Test performance guardrail: do not partial-mock broad `soloclaw/plugin-sdk/*` barrels in hot tests. Add a plugin-local `*.runtime.ts` seam and mock that seam instead.
...
- Vitest now defaults to native root-project `threads`, with hard `forks` exceptions for `gateway`, `agents`, and `commands`. Keep new pool changes explicit and justified; use `SOLOCLAW_VITEST_POOL=forks` for full local fork debugging.
- If local Vitest runs cause memory pressure, the default worker budget now derives from host capabilities (CPU, memory band, current load). For a conservative explicit override during land/gate runs, use `SOLOCLAW_VITEST_MAX_WORKERS=1 pnpm test`.
- Live tests (real keys): `SOLOCLAW_LIVE_TEST=1 pnpm test:live` (OpenClaw-only) or `LIVE=1 pnpm test:live` (includes provider live tests). Docker: `pnpm test:docker:live-models`, `pnpm test:docker:live-gateway`. Onboarding Docker E2E: `pnpm test:docker:onboard`.
- `pnpm test:live` defaults quiet now. Keep `[live]` progress; suppress profile/gateway chatter. Full logs: `SOLOCLAW_LIVE_TEST_QUIET=0 pnpm test:live`.
...
- Channel/provider state lives under `~/.soloclaw/credentials/`; rerun `soloclaw channels login` if logged out. Model auth profiles live under `~/.soloclaw/agents/<agentId>/agent/auth-profiles.json`; legacy OAuth import still reads `~/.soloclaw/credentials/oauth.json`.
- Pi sessions live under `~/.soloclaw/agents/<agentId>/sessions/` by default; `session.store` can override the session store path.
...
- Rebrand/migration issues or legacy config/service warnings: run `soloclaw doctor` (see `docs/gateway/doctor.md`).
...
- Gateway may run as an app-managed launchd job. Restart the gateway via the app or `soloclaw gateway restart`; inspect with `soloclaw gateway status --deep` or, for the default profile, `launchctl print gui/$UID/ai.soloclaw.gateway`. Use `scripts/restart-mac.sh` when you need to rebuild/relaunch the local macOS app itself. The app LaunchAgent uses `ai.soloclaw.mac`. **When debugging on macOS, start/stop the gateway via the app or gateway CLI, not ad-hoc tmux sessions; kill any temporary tunnels before handoff.**
- macOS logs: use `./scripts/clawlog.sh` to query unified logs for the SoloClaw subsystem; it supports follow/tail/category filters and expects passwordless sudo for `/usr/bin/log`.
...
- When asked to open a “session” file, open the Pi session logs under `~/.soloclaw/agents/<agentId>/sessions/*.jsonl` (use the `agent=<id>` value in the Runtime line of the system prompt; newest unless a specific ID is given), not the default `sessions.json`. If logs are needed from another machine, SSH via Tailscale and read the same path there.
...
- For manual `soloclaw message send` messages that include `!`, use the heredoc pattern noted below to avoid the Bash tool’s escaping.
