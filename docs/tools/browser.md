...
SoloClaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
Set `browser.defaultProfile: "soloclaw"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.soloclaw/soloclaw.json`.
...
- If you override the Gateway port (`gateway.port` or `SOLOCLAW_GATEWAY_PORT`),
...
SoloClaw uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, SoloClaw will not launch a local browser.
...
SoloClaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, SoloClaw can
...
- If you set `nodeHost.browserProxy.allowProfiles`, SoloClaw treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. SoloClaw can use either form, but
...
the standard HTTP-based CDP discovery (`/json/version`). SoloClaw supports both:

- **HTTP(S) endpoints** — SoloClaw calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — SoloClaw connects directly,
...
- SoloClaw does **not** auto-generate that token when `gateway.auth.mode` is
...
SoloClaw supports multiple named profiles (routing configs). Profiles can be:
...
SoloClaw can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when SoloClaw attaches.
...
- SoloClaw does not launch the browser for this driver; it attaches to an
  existing session only.
- SoloClaw uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, SoloClaw passes it through to target that explicit
...
When launching locally, SoloClaw picks the first available:
...
`SOLOCLAW_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- Download and trace output paths are constrained to SoloClaw temp roots:
  - traces: `/tmp/openclaw` (fallback: `${os.tmpdir()}/openclaw`)
  - downloads: `/tmp/openclaw/downloads` (fallback: `${os.tmpdir()}/openclaw/downloads`)
- Upload paths are constrained to an SoloClaw temp uploads root:
...
SoloClaw supports two “snapshot” styles:
...
- **CDP startup or readiness failure** means SoloClaw cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "soloclaw" is not reachable after start`
