...
SoloClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, SoloClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
`~/.soloclaw/soloclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.soloclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
