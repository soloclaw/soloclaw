...
SoloClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), SoloClaw reuses that same host for Kimi
...
If you omit `baseUrl`, SoloClaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, SoloClaw defaults to `kimi-k2.5`.
