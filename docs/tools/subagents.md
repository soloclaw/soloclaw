...
- On completion, SoloClaw best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.
- For manual spawns, delivery is resilient:
  - SoloClaw tries direct `agent` delivery first with a stable idempotency key.
...
- Completion delivery keeps the resolved requester route:
  ...
  - if the completion origin only provides a channel, SoloClaw fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, SoloClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. SoloClaw creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, SoloClaw falls back to that session's requester when available
