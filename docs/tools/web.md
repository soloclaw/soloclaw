...
SoloClaw also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "SoloClaw plugin SDK" });
    ```
...
If native Codex search is enabled but the current model is not Codex-capable, SoloClaw keeps the normal managed `web_search` behavior.
...
If no `provider` is set, SoloClaw checks providers in this order and uses the
...
<Note>
  ...
  SoloClaw resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let SoloClaw auto-detect the first ready web-fetch
...
`soloclaw configure --section web`, SoloClaw can also ask for:
...
SoloClaw can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, SoloClaw does not
...
optional structured filters. SoloClaw only enables the built-in xAI `x_search`
