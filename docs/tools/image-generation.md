...
Not all providers support all parameters. When a fallback provider supports a nearby geometry option instead of the exact requested one, SoloClaw remaps to the closest supported size, aspect ratio, or resolution before submission. Truly unsupported overrides are still reported in the tool result.

Tool results report the applied settings. When SoloClaw remaps geometry during provider fallback, the returned `size`, `aspectRatio`, and `resolution` values reflect what was actually sent, and `details.normalization` captures the requested-to-applied translation.
...
When generating an image, SoloClaw tries providers in this order:
...
- Auto-detection is auth-aware. A provider default only enters the candidate list
  when SoloClaw can actually authenticate that provider.
