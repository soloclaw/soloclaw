...
The tool is only registered when SoloClaw can resolve a PDF-capable model config for the agent:
...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  SoloClaw can actually authenticate that provider for the agent.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, SoloClaw drops the rendered images and continues with the
  extracted text.
