...
- Provider notes:
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; SoloClaw normalizes incompatible values to `auto`.
...
- SoloClaw resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode sends the same `service_tier=priority` flag on Codex Responses. SoloClaw keeps one shared `/fast` toggle across both auth paths.
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. SoloClaw still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
