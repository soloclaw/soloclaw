---
summary: "SoloClaw tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools SoloClaw provides
...
SoloClaw has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). SoloClaw ships a set of **built-in tools** and
...
These tools ship with SoloClaw and are available without installing any plugins:
...
| `group:openclaw`   | All built-in SoloClaw tools (excludes plugin tools)                                                       |
