...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In SoloClaw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
Dev/local checkout: `soloclaw plugins install ./path/to/local/open-prose-plugin`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses SoloClaw tools under the hood.
...
## SoloClaw runtime mapping

OpenProse programs map to SoloClaw primitives:

| OpenProse concept         | SoloClaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use SoloClaw tool allowlists and approval gates to control side effects.
