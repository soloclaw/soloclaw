---
title: "Codex Harness"
summary: "Run SoloClaw embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets SoloClaw run embedded agent turns through the
...
SoloClaw still owns chat channels, session files, model selection, tools,
...
SoloClaw has separate routes for OpenAI and Codex-shaped access:
...
- SoloClaw with the bundled `codex` plugin available.
...
SoloClaw on the protocol surface it has been tested against.
...
With fallback disabled, SoloClaw fails early if the Codex plugin is disabled,
...
SoloClaw session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the SoloClaw session binding for that thread.
...
Model switching stays OpenClaw-controlled. When an SoloClaw session is attached
...
generic and works on any channel that supports SoloClaw text commands.
...
- `/codex resume <thread-id>` attaches the current SoloClaw session to an existing Codex thread.
...
normal turns. On the next message, SoloClaw resumes that Codex thread, passes the
currently selected SoloClaw `codex/*` model into app-server, and keeps extended
history enabled.
...
SoloClaw still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal SoloClaw delivery path.

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. SoloClaw keeps a transcript mirror for channel
...
**SoloClaw falls back to PI:** set `embeddedHarness.fallback: "none"` or
