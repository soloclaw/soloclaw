...
read_when:
  - You are building an SoloClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (soloclaw.plugin.json)

This page is for the **native SoloClaw plugin manifest** only.
...
SoloClaw auto-detects those bundle layouts too, but they are not validated
against the `soloclaw.plugin.json` schema described here.

For compatible bundles, SoloClaw currently reads bundle metadata plus declared
...
SoloClaw runtime expectations.

Every native SoloClaw plugin **must** ship a `soloclaw.plugin.json` file in the
**plugin root**. SoloClaw uses this manifest to validate configuration
...
`soloclaw.plugin.json` is the metadata SoloClaw reads before it loads your
plugin code.
...
- static capability ownership snapshots used for bundled compat wiring and
  contract coverage
- cheap QA runner metadata that the shared `soloclaw qa` host can inspect
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that SoloClaw can inspect without loading plugin code.                                                                                                                      |
| `providerAuthAliases`               | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                   |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that SoloClaw can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
| `qaRunners`                         | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `soloclaw qa` host before plugin runtime loads.                                                                                                               |
...
SoloClaw reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, SoloClaw falls back to `choiceId`.                                        |
...
the shared `soloclaw qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `soloclaw qa`, for example `matrix`.    |
...
Use `contracts` only for static capability ownership metadata that SoloClaw can
...
Use `modelSupport` when SoloClaw should infer your provider plugin from
...
SoloClaw applies this precedence:
...
Legacy top-level capability keys are deprecated. Use `soloclaw doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `soloclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
...
- if SoloClaw must know it before loading plugin code, put it in `soloclaw.plugin.json`
...
`openclaw` block instead of `soloclaw.plugin.json`.
...
| `openclaw.install.minHostVersion`                                 | Minimum supported SoloClaw host version, using a semver floor like `>=2026.3.22`.                                                            |
...
- The manifest is **required for native SoloClaw plugins**, including local filesystem loads.
