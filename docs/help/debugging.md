...
- **Global `--dev` (profile):** isolates state under `~/.soloclaw-dev` and
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.soloclaw/logs/raw-stream.jsonl
```
...
`~/.soloclaw/logs/raw-stream.jsonl`
