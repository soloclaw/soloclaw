...
## Enable for OpenClaw (`ai.soloclaw`)

- Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.soloclaw.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.soloclaw.plist /Library/Preferences/Logging/Subsystems/ai.soloclaw.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.soloclaw.plist`.
