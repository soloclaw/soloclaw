...
- The app installs a per‑user LaunchAgent labeled `ai.soloclaw.gateway`
  (or `ai.soloclaw.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.soloclaw.*` is supported).
