...
- Re-open anytime: `soloclaw dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `SOLOCLAW_GATEWAY_TOKEN`); `soloclaw dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `soloclaw dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `soloclaw dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `SOLOCLAW_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `soloclaw status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `soloclaw config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `SOLOCLAW_GATEWAY_TOKEN` in this shell, then rerun `soloclaw dashboard`
  - No shared secret configured: `soloclaw doctor --generate-gateway-token`
