...
```bash
soloclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `soloclaw tui --deliver`
...
- Local shell commands receive `SOLOCLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `SOLOCLAW_THEME=light` before launching `soloclaw tui`.
- To force the original dark palette instead, set `SOLOCLAW_THEME=dark`.
...
- Check the Gateway logs: `soloclaw logs --follow`.
- Confirm the agent can run: `soloclaw status` and `soloclaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `soloclaw agents list` and your routing config.
