...
- `/usage cost` in chats: local cost summary aggregated from SoloClaw session logs.
- CLI: `soloclaw status --usage` prints a full per-provider breakdown.
- CLI: `soloclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. SoloClaw treats
  ...
  quota, so SoloClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, SoloClaw prefers the
...
can supply plugin-specific usage auth logic; otherwise SoloClaw falls back to
