---
summary: "How SoloClaw rotates auth profiles and falls back across models"
read_when:
...
SoloClaw handles failures in two stages:
...
For a normal text run, SoloClaw evaluates candidates in this order:
...
Inside each candidate, SoloClaw tries auth-profile failover before advancing to
...
SoloClaw uses **auth profiles** for both API keys and OAuth tokens.
...
When a provider has multiple profiles, SoloClaw chooses an order like this:

1. **Explicit config**: `auth.order[provider]` (if set).
2. **Configured profiles**: `auth.profiles` filtered by provider.
3. **Stored profiles**: entries in `auth-profiles.json` for the provider.

If no explicit order is configured, SoloClaw uses a round‑robin order:
...
SoloClaw **pins the chosen auth profile per session** to keep provider caches warm.
...
they are tried first, but SoloClaw may rotate to another profile on rate limits/timeouts.
User‑pinned profiles stay locked to that profile; if it fails and model fallbacks
are configured, SoloClaw moves to the next model instead of switching profiles.
...
like rate limiting), SoloClaw marks it in cooldown and moves to the next profile.
...
- SoloClaw records `cooldownModel` for rate-limit failures when the failing
  model id is known.
...
Billing/credit failures (for example “insufficient credits” / “credit balance too low”) are treated as failover‑worthy, but they’re usually not transient. Instead of a short cooldown, SoloClaw marks the profile as **disabled** (with a longer backoff) and rotates to the next profile/provider.

Not every billing-shaped response is `402`, and not every HTTP `402` lands
here. SoloClaw keeps explicit billing text in the billing lane even when a
...
If all profiles for a provider fail, SoloClaw moves to the next model in
`agents.defaults.model.fallbacks`. This applies to auth failures, rate limits, and
timeouts that exhausted profile rotation (other errors do not advance fallback).

Overloaded and rate-limit errors are handled more aggressively than billing
cooldowns. By default, SoloClaw allows one same-provider auth-profile retry,
...
SoloClaw builds the candidate list from the currently requested `provider/model`
...
- If the current run is already on a configured fallback in the same provider
  family, SoloClaw keeps using the full configured chain.
- If the current run is on a different provider than config and that current
  model is not already part of the configured fallback chain, SoloClaw does not
...
When every auth profile for a provider is already in cooldown, SoloClaw does
...
When every candidate fails, SoloClaw throws `FallbackSummaryError`. The outer
