...
SoloClaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models SoloClaw can use (plus aliases).
...
- If you omit the provider, SoloClaw resolves the input in this order:
  1. alias match
...
agent directory (default `~/.soloclaw/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: SoloClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever SoloClaw regenerates `models.json`, including command-driven paths like `soloclaw agent`.
