---
summary: "How SoloClaw builds prompt context and reports token usage + costs"
read_when:
...
SoloClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SoloClaw assembles its own system prompt on every run. It includes:
...
For images, SoloClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from SoloClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `soloclaw status --usage` and `soloclaw channels list` show
...
available; otherwise SoloClaw falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, SoloClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. SoloClaw can
...
Anthropic's 1M context window is currently beta-gated. SoloClaw can inject the
...
SoloClaw skips the `context-1m-*` beta header because Anthropic currently
