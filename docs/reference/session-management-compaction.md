...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.soloclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- `soloclaw status` (CLI)
- `soloclaw sessions` / `sessions --json`
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `soloclaw status`.
