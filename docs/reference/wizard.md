...
This is the full reference for `soloclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.soloclaw/soloclaw.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `soloclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though SoloClaw now prefers Claude CLI reuse when available.
    - **OpenAI Code (Codex) subscription (Codex CLI)**: if `~/.codex/auth.json` exists, onboarding can reuse it. Reused Codex CLI credentials stay managed by Codex CLI; on expiry SoloClaw re-reads that source first and, when the provider can refresh it, writes the refreshed credential back to Codex storage instead of taking ownership itself.
    ...
    - Auth profiles live in `~/.soloclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.soloclaw/credentials/oauth.json` is legacy import-only.
  ...
  <Step title="Workspace">
    - Default `~/.soloclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `soloclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `soloclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `soloclaw health`.
    - Tip: `soloclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.soloclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.soloclaw/soloclaw.json`:
...
`soloclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.soloclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.soloclaw/agents/<agentId>/sessions/`.
