...
SoloClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SoloClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- SoloClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
