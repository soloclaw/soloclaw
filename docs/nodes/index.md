...
- **Approvals**: enforced on the node host via `~/.soloclaw/exec-approvals.json`.
...
- `--display-name` on `openclaw node run` / `openclaw node install` (persists in `~/.soloclaw/node.json` on the node).
...
Approvals live on the node host at `~/.soloclaw/exec-approvals.json`.
...
- On headless node host, `system.run` is gated by exec approvals (`~/.soloclaw/exec-approvals.json`).
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.soloclaw/node.json`.
- Exec approvals are enforced locally via `~/.soloclaw/exec-approvals.json`
