---
summary: "CLI reference for `soloclaw onboard` (interactive onboarding)"
read_when:
...
```bash
soloclaw onboard
soloclaw onboard --flow quickstart
soloclaw onboard --flow manual
soloclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `soloclaw gateway run`.
...
- Fastest first chat: `soloclaw dashboard` (Control UI, no channel setup).
