...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an SoloClaw Gateway.
...
If you want an external MCP client to talk directly to SoloClaw channel
...
- SoloClaw acts as an ACP server
- an IDE or ACP client connects to OpenClaw
- SoloClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where SoloClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to OpenClaw: use `soloclaw acp`
- SoloClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the SoloClaw gateway or agent instead.                                                                                                                                     |
...
openclaw acp --url wss://gateway-host:18789 --token-file ~/.soloclaw/gateway.token
...
openclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.soloclaw/gateway.token
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run SoloClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an SoloClaw Gateway session.
...
If you want ACPX-backed sessions to see SoloClaw plugin tools, enable the
...
SoloClaw bot over ACP, use `acpx` with its built-in `openclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx openclaw` at `soloclaw acp`.
3. Target the SoloClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default SoloClaw ACP session
acpx openclaw exec "Summarize the active SoloClaw session state."

# Persistent named session for follow-up turns
acpx openclaw sessions ensure --name codex-bridge
acpx openclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my SoloClaw work agent for recent context relevant to this repo."
```
...
```json
{
  "agents": {
    "soloclaw": {
      "command": "env SOLOCLAW_HIDE_BANNER=1 SOLOCLAW_SUPPRESS_NOTES=1 openclaw acp --url ws://127.0.0.1:18789 --token-file ~/.soloclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local SoloClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env SOLOCLAW_HIDE_BANNER=1 SOLOCLAW_SUPPRESS_NOTES=1 node openclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an SoloClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “SoloClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`SOLOCLAW_GATEWAY_TOKEN`, `SOLOCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`SOLOCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `SOLOCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `soloclaw acp client` sets `SOLOCLAW_SHELL=acp-client` on the spawned bridge process.
