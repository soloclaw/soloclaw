---
summary: "SoloClaw Gateway CLI (`soloclaw gateway`) — run, query, and discover gateways"
read_when:
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.soloclaw/soloclaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
- `--token <token>`: token override (also sets `SOLOCLAW_GATEWAY_TOKEN` for the process).
...
- For password auth on `gateway run`, prefer `SOLOCLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `SOLOCLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
