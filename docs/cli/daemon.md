---
summary: "CLI reference for `soloclaw daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `soloclaw daemon ...` in scripts
...
`soloclaw daemon ...` maps to the same service control surface as `soloclaw gateway ...` service commands.
...
Use [`soloclaw gateway`](/cli/gateway) for current docs and examples.
