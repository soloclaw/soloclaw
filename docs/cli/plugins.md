...
Native SoloClaw plugins must ship `soloclaw.plugin.json` with an inline JSON
...
those to a prerelease, SoloClaw stops and asks you to opt in explicitly with a
...
SoloClaw now also prefers ClawHub for bare npm-safe plugin specs. It only falls
...
SoloClaw downloads the package archive from ClawHub, checks the advertised
...
inside the cloned marketplace repo. SoloClaw accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, SoloClaw auto-detects:

- native SoloClaw plugins (`soloclaw.plugin.json`)
...
When you pass a plugin id, SoloClaw reuses the recorded install spec for that
...
or exact version. SoloClaw resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
SoloClaw prints a warning and asks for confirmation before proceeding. Use
