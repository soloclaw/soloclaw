---
summary: "CLI reference for `soloclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `soloclaw config` without a subcommand opens the same wizard. Use
`soloclaw config get|set|unset` for non-interactive edits.

For web search, `soloclaw configure --section web` lets you choose a provider
...
soloclaw configure --section model --section channels
soloclaw configure --section gateway --section daemon
```
