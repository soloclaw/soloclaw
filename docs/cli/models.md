...
the command uses `SOLOCLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `SOLOCLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
