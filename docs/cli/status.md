---
summary: "CLI reference for `soloclaw status` (diagnostics, probes, usage snapshots)"
read_when:
...
- MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota, so SoloClaw inverts them before display; count-based fields win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `soloclaw update` (see [Updating](/install/updating)).
