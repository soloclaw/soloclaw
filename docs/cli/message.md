...
- On cache miss, SoloClaw will attempt a live directory lookup when the provider supports it.
...
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, SoloClaw does not force a `default` account SecretRef scope
