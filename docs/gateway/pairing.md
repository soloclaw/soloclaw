...
```bash
soloclaw nodes pending
soloclaw nodes approve <requestId>
soloclaw nodes reject <requestId>
soloclaw nodes status
soloclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.soloclaw`):
