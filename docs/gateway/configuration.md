...
SoloClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.soloclaw/soloclaw.json`.

If the file is missing, SoloClaw uses safe defaults. Common reasons to add a config:
...
<Warning>
SoloClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    ...
    - `SOLOCLAW_APNS_RELAY_BASE_URL` and `SOLOCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `SOLOCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
SoloClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.soloclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, SoloClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `SOLOCLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${SOLOCLAW_GATEWAY_TOKEN}" } },
