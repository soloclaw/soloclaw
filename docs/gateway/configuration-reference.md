...
- `soloclaw config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.soloclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `soloclaw doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `soloclaw doctor` warns when this is missing or invalid.
...
- `config: true` enables `/config` (reads/writes `soloclaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
- Environment overrides: `SOLOCLAW_AGENT_RUNTIME=<id|auto|pi>` overrides `runtime`; `SOLOCLAW_AGENT_HARNESS_FALLBACK=none` disables PI fallback for that process.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.soloclaw/sandboxes",
        docker: {
...
- `none`: per-scope sandbox workspace under `~/.soloclaw/sandboxes`
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from SOLOCLAW_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `SOLOCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `SOLOCLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `SOLOCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.soloclaw/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.soloclaw/workspace-home" },
      { id: "work", workspace: "~/.soloclaw/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.soloclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.soloclaw/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.soloclaw/workspace-public",
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.soloclaw/settings/tts.json",
      elevenlabs: {
...
- Files are materialized into the child workspace at `.soloclaw/attachments/<uuid>/` with a `.manifest.json`.
...
- Override agent config root with `SOLOCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `soloclaw onboard --auth-choice opencode-zen` or `soloclaw onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `soloclaw onboard --auth-choice zai-api-key`.
...
Anthropic-compatible, built-in provider. Shortcut: `soloclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `soloclaw onboard --auth-choice synthetic-api-key`.
...
- `install.nodeManager`: node installer preference for `metadata.soloclaw.install`
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SOLOCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SOLOCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `SOLOCLAW_APNS_RELAY_BASE_URL` / `SOLOCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `SOLOCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
SOLOCLAW_CONFIG_PATH=~/.soloclaw/a.json \
SOLOCLAW_STATE_DIR=~/.soloclaw-a \
soloclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.soloclaw-dev` + port `19001`), `--profile <name>` (uses `~/.soloclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.soloclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SOLOCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.soloclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or SOLOCLAW_SKIP_CANVAS_HOST=1
  },
}
```
...
- Hostname defaults to `openclaw`. Override with `SOLOCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.soloclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `soloclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.soloclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SOLOCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `soloclaw.json` credential paths.
...
- Legacy OAuth imports from `~/.soloclaw/credentials/oauth.json`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.soloclaw/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$SOLOCLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- To hide the entire banner (not just taglines), set env `SOLOCLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `soloclaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `soloclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
