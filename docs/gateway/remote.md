...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “SoloClaw runs”).
...
Note: replace `18789` with your configured `gateway.port` (or `--port`/`SOLOCLAW_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`SOLOCLAW_GATEWAY_URL`) may use env credentials only (`SOLOCLAW_GATEWAY_TOKEN` / `SOLOCLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `SOLOCLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `SOLOCLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `SOLOCLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `SOLOCLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `SOLOCLAW_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
