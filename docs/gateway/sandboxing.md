...
- `"none"` (default): tools see a sandbox workspace under `~/.soloclaw/sandboxes`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `SOLOCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `SOLOCLAW_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `SOLOCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `SOLOCLAW_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `SOLOCLAW_DOCKER_SOCKET`. Full setup and env
