...
- During heartbeat runs, SoloClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `soloclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, SoloClaw can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, SoloClaw skips the heartbeat run entirely (no model call).
...
- SoloClaw parses the `tasks:` block and checks each task against its own `interval`.
