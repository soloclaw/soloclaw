...
SoloClaw has three related (but different) controls:
...
Use the inspector to see what SoloClaw is _actually_ doing:
...
- SoloClaw validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:openclaw`: all built-in SoloClaw tools (excludes provider plugins)
