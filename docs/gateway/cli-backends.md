...
SoloClaw can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **SoloClaw tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
soloclaw agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, SoloClaw now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), SoloClaw will
...
2. **Builds a system prompt** using the same SoloClaw prompt + workspace context.
...
told us OpenClaw-style Claude CLI usage is allowed again, so SoloClaw treats
...
`--append-system-prompt` flag, so SoloClaw writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the SoloClaw skills snapshot
two ways: the compact SoloClaw skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that SoloClaw would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by SoloClaw to the
...
- SoloClaw drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
SoloClaw will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, SoloClaw appends the
...
- For Gemini CLI JSON output, SoloClaw reads reply text from `response` and
...
- `stats.cached` is normalized into SoloClaw `cacheRead`.
- If `stats.input` is missing, SoloClaw derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before SoloClaw handles
...
CLI backends do **not** receive SoloClaw tool calls directly, but a backend can
...
If no MCP servers are enabled, SoloClaw still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct SoloClaw tool calls.** SoloClaw does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. SoloClaw sessions still work
  normally.
