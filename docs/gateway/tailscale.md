...
SoloClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. SoloClaw requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. SoloClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
SoloClaw only treats a request as Serve when it arrives from loopback with
...
soloclaw gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want SoloClaw to undo `tailscale serve`
