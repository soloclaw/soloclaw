...
- `SOLOCLAW_CONFIG_PATH` — per-instance config file
- `SOLOCLAW_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `SOLOCLAW_STATE_DIR` + `SOLOCLAW_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `SOLOCLAW_GATEWAY_PORT` / `--port`).
...
```bash
SOLOCLAW_CONFIG_PATH=~/.soloclaw/main.json \
SOLOCLAW_STATE_DIR=~/.soloclaw-main \
soloclaw gateway --port 18789

SOLOCLAW_CONFIG_PATH=~/.soloclaw/rescue.json \
SOLOCLAW_STATE_DIR=~/.soloclaw-rescue \
