---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for SoloClaw agents"
read_when:
...
containers locally, SoloClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- SoloClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, SoloClaw creates an OpenShell
...
- You edit files locally outside SoloClaw and want those changes visible in the
...
- When the sandbox is first created, SoloClaw seeds the remote workspace from
...
- SoloClaw does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside SoloClaw after the initial seed,
...
| `from`                    | `string`                 | `"soloclaw"`  | Sandbox source for first-time create                  |
...
1. SoloClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. SoloClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
