...
read_when:
  - Running SoloClaw behind an identity-aware proxy
...
- You run SoloClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SoloClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SoloClaw extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that SoloClaw can verify.
...
- SoloClaw can stay on loopback HTTP behind the proxy.
...
If SoloClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
SoloClaw rejects ambiguous configurations where both a `gateway.auth.token` (or `SOLOCLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
- When the header is present, SoloClaw honors the declared scope set.
...
`soloclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
SoloClaw rejected a loopback-source trusted-proxy request.
...
3. Update SoloClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `soloclaw security audit` and review findings
