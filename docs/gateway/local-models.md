---
summary: "Run SoloClaw on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
Local is doable, but SoloClaw expects large context + strong defenses against prompt injection. Small cards truncate context and leak safety. Aim high: **≥2 maxed-out Mac Studios or equivalent GPU rig (~$30k+)**. A single **24 GB** GPU works only for lighter prompts with higher latency. Use the **largest / full-size model variant you can run**; aggressively quantized or “small” checkpoints raise prompt-injection risk (see [Security](/gateway/security)).

If you want the lowest-friction local setup, start with [LM Studio](/providers/lmstudio) or [Ollama](/providers/ollama) and `soloclaw onboard`. This page is the opinionated guide for higher-end local stacks and custom OpenAI-compatible local servers.
...
- SoloClaw treats these as proxy-style OpenAI-compatible routes, not native
  OpenAI endpoints
...
- hidden SoloClaw attribution headers (`originator`, `version`, `User-Agent`)
...
- Some smaller or stricter local backends are unstable with OpenClaw's full
  ...
  SoloClaw agent turns, first try
...
- If the backend still fails only on larger SoloClaw runs, the remaining issue
...
- SoloClaw warns when the detected context window is below **32k** and blocks below **16k**. If you hit that preflight, raise the server/model context limit or choose a larger model.
- Context errors? Lower `contextWindow` or raise your server limit.
- OpenAI-compatible server returns `messages[].content ... expected a string`?
  Add `compat.requiresStringContent: true` on that model entry.
- Direct tiny `/v1/chat/completions` calls work, but `soloclaw infer model run`
  ...
  on larger SoloClaw prompts, treat it as an upstream server/model limitation.
