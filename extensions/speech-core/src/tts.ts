...
import { resolvePreferredSoloClawTmpDir } from "soloclaw/plugin-sdk/sandbox";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.SOLOCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: SoloClawConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: SoloClawConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: SoloClawConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: SoloClawConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: SoloClawConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: SoloClawConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredSoloClawTmpDir();
