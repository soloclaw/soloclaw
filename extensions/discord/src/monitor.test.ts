...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("soloclaw/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of SoloClaw")).toBe("friends-of-openclaw");
  ...
  it("matches ids by default and names only when enabled", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of SoloClaw"],
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    ...
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SoloClaw"),
      guildEntries,
    });
...
const channelRuntimeModule = await import("soloclaw/plugin-sdk/infra-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("soloclaw/plugin-sdk/routing");
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    cfg: {} as ReturnType<typeof import("soloclaw/plugin-sdk/config-runtime").loadConfig>,
    accountId: "acc-1",
    runtime: {} as import("soloclaw/plugin-sdk/runtime-env").RuntimeEnv,
