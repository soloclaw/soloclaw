import { DEFAULT_ACCOUNT_ID } from "soloclaw/plugin-sdk/account-id";
import type { DiscordGuildEntry, SoloClawConfig } from "soloclaw/plugin-sdk/config-runtime";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "soloclaw/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "soloclaw/plugin-sdk/setup-runtime";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: SoloClawConfig; accountId: string }) => {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: SoloClawConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: SoloClawConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: SoloClawConfig; accountId: string }) =>
    ...
    disable: (cfg: SoloClawConfig) => setSetupChannelEnabled(cfg, channel, false),
