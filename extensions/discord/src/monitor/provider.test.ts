...
import { AcpRuntimeError } from "soloclaw/plugin-sdk/acp-runtime";
...
let runtimeEnvModule: typeof import("soloclaw/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): SoloClawConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("soloclaw/plugin-sdk/plugin-runtime", async () => {
      const actual = await vi.importActual<typeof import("soloclaw/plugin-sdk/plugin-runtime")>(
        "soloclaw/plugin-sdk/plugin-runtime",
      );
      return {
        ...actual,
    ...
    runtimeEnvModule = await import("soloclaw/plugin-sdk/runtime-env");
