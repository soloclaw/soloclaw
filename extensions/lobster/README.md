...
- This plugin integrates Lobster with SoloClaw _without core changes_.
...
## Using `openclaw.invoke` (Lobster → SoloClaw tools)

Some Lobster pipelines may include a `openclaw.invoke` step to call back into SoloClaw tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the SoloClaw Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- SoloClaw provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, SoloClaw returns `404 Tool not available`.
