...
metadata: { "soloclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SoloClaw `sessions_spawn`
- **File I/O** == SoloClaw `read`/`write`
- **Remote fetch** == SoloClaw `web_fetch` (or `exec` with curl when POST is required)
