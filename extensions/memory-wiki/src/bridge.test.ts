...
} from "soloclaw/plugin-sdk/memory-host-events";
...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("imports public memory artifacts and stays idempotent across reruns", async () => {
    ...
    const appConfig: SoloClawConfig = {
      agents: {
    ...
    const logLines = (await fs.readFile(path.join(vaultDir, ".soloclaw-wiki", "log.jsonl"), "utf8"))
      .trim()
      .split("\n");
