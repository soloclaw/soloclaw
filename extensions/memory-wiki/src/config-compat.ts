...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.memory-wiki.config.bridge.readMemoryCore is legacy; use plugins.entries.memory-wiki.config.bridge.readMemoryArtifacts. Run "soloclaw doctor --fix".',
    match: hasLegacyBridgeArtifactToggle,
  },
];

export function migrateMemoryWikiLegacyConfig(config: SoloClawConfig): {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: SoloClawConfig }): {
  config: SoloClawConfig;
  changes: string[];
} {
