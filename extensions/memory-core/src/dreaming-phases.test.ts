...
const LIGHT_DREAMING_TEST_CONFIG: SoloClawConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__soloclaw_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __soloclaw_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__soloclaw_memory_core_light_sleep__",
            "",
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    const workspaceDir = await createDreamingWorkspace();
    vi.stubEnv("SOLOCLAW_TEST_FAST", "1");
    vi.stubEnv("SOLOCLAW_STATE_DIR", path.join(workspaceDir, ".state"));
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__soloclaw_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: SoloClawConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
