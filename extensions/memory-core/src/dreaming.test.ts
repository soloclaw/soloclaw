...
describe("short-term dreaming cron reconciliation", () => {
  ...
  it("migrates legacy light/rem dreaming cron jobs during reconciliation", async () => {
    ...
    const legacyLightJob: CronJobLike = {
      ...
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "__soloclaw_memory_core_light_sleep__" },
      createdAtMs: 8,
    };
    const legacyRemJob: CronJobLike = {
      ...
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "__soloclaw_memory_core_rem_sleep__" },
      createdAtMs: 9,
    };
...
describe("short-term dreaming trigger", () => {
  ...
  it("applies promotions when the managed dreaming token is embedded in a reminder body", async () => {
    ...
    const result = await runShortTermDreamingPromotionIfTriggered({
      cleanedBody: [
        "System: rotate logs",
        "System: __soloclaw_memory_core_short_term_promotion_dream__",
        "",
        "A scheduled reminder has been triggered. The reminder content is:",
        "",
        "rotate logs",
        "__soloclaw_memory_core_short_term_promotion_dream__",
        "",
