...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("soloclaw memory status --fix");
    expect(helpText).toContain("Repair stale recall locks and normalize promotion metadata.");
    expect(helpText).toContain("soloclaw memory status --deep");
    ...
    expect(helpText).toContain("soloclaw memory promote --apply");
    ...
    expect(helpText).toContain("soloclaw memory rem-harness --json");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## SoloClaw / runtime / workflow preferences and corrections",
