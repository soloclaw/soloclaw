...
function resolveStoredDreaming(config: SoloClawConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: SoloClawConfig = {}) {
  const registered: { command?: SoloClawPluginCommandDefinition } = {};
  let runtimeConfig: SoloClawConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: SoloClawConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: SoloClawPluginCommandDefinition) => {
      registered.command = definition;
    }),
  } as unknown as SoloClawPluginApi;
