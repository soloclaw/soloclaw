...
describe("memory watcher config", () => {
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): SoloClawConfig {
    const defaults: NonNullable<NonNullable<SoloClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: SoloClawConfig) {
