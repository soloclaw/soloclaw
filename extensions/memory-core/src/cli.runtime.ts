...
function resolveMemoryPluginConfig(cfg: SoloClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredSoloClawTmpDir(), "openclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: SoloClawConfig): string {
...
function resolveAgent(cfg: SoloClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: SoloClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredSoloClawTmpDir(), "openclaw-rem-backfill-"),
      );
      try {
