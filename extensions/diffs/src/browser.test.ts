...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<SoloClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: SoloClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<SoloClawPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as SoloClawPluginApi);
