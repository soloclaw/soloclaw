...
const SOLOCLAW_OPENSHELL_E2E = process.env.SOLOCLAW_E2E_OPENSHELL === "1";
const SOLOCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const SOLOCLAW_OPENSHELL_COMMAND =
  process.env.SOLOCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && SOLOCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: SOLOCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(SOLOCLAW_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(SOLOCLAW_OPENSHELL_COMMAND))) {
        return;
      }
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: SOLOCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
