...
afterEach(async () => {
  ...
  delete process.env.SOLOCLAW_SKIP_ACPX_RUNTIME;
  delete process.env.SOLOCLAW_SKIP_ACPX_RUNTIME_PROBE;
...
function createServiceContext(workspaceDir: string) {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".soloclaw-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe("createAcpxRuntimeService", () => {
  ...
  it("can skip the embedded runtime probe via env", async () => {
    process.env.SOLOCLAW_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const service = createAcpxRuntimeService({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.SOLOCLAW_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (SOLOCLAW_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
