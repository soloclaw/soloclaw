...
The bundled ACPX extension is a thin SoloClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `openclaw/acpx`, not in this extension.
...
Use this flow when SoloClaw needs unreleased ACPX changes before the ACPX version is published.
...
6. Rebuild SoloClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for SoloClaw ACP validation.
