...
`openclaw.invoke --each`) without adding new SoloClaw code per workflow.
...
This extension depends on SoloClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SoloClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.soloclaw/extensions` as a standalone plugin directory.
