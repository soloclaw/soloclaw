import type { GetReplyOptions, MsgContext } from "soloclaw/plugin-sdk/reply-runtime";
import { withEnvAsync } from "soloclaw/plugin-sdk/testing";
import { afterAll, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { escapeRegExp, formatEnvelopeTimestamp } from "../../../test/helpers/envelope-timestamp.js";
const harness = await import("./bot.create-telegram-bot.test-harness.js");
const conversationRuntime = await import("soloclaw/plugin-sdk/conversation-runtime");
const configRuntime = await import("soloclaw/plugin-sdk/config-runtime");
...
describe("createTelegramBot", () => {
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("soloclaw pairing approve telegram");
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "SoloClaw", username: "soloclaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
      ...
      me: { id: 7, is_bot: true, first_name: "SoloClaw", username: "soloclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "soloclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
