...
import { createLazyChannelApprovalNativeRuntimeAdapter } from "soloclaw/plugin-sdk/approval-handler-adapter-runtime";
import type { ChannelApprovalNativeRuntimeAdapter } from "soloclaw/plugin-sdk/approval-handler-runtime";
import {
  createChannelApproverDmTargetResolver,
  createChannelNativeOriginTargetResolver,
} from "soloclaw/plugin-sdk/approval-native-runtime";
import type { ChannelApprovalCapability } from "soloclaw/plugin-sdk/channel-contract";
  ...
  describeExecApprovalSetup: ({ accountId }: { accountId?: string | null }) => {
    const prefix =
      accountId && accountId !== "default"
        ...
        : "channels.telegram";
    return `Approve it from the Web UI or terminal UI for now. Telegram supports native exec approvals for this account. Configure \`${prefix}.execApprovals.approvers\`; if you leave it unset, SoloClaw can infer numeric owner IDs from \`${prefix}.allowFrom\` or direct-message \`${prefix}.defaultTo\` when possible. Leave \`${prefix}.execApprovals.enabled\` unset/\`auto\` or set it to \`true\`.`;
  },
