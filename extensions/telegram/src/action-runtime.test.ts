...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SoloClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SoloClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SoloClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SoloClawConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: SoloClawConfig, accountId = "media") {
    await handleTelegramAction(
