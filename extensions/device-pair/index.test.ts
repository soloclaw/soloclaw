...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredSoloClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "openclaw-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredSoloClawTmpDir: pluginApiMocks.resolvePreferredSoloClawTmpDir,
...
function createApi(params?: {
  runtime?: SoloClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as SoloClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: SoloClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as SoloClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredSoloClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredSoloClawTmpDir(), { recursive: true, force: true });
  });
