...
vi.mock("soloclaw/plugin-sdk/browser-config-runtime", () => ({
  loadConfig: configMocks.loadConfig,
}));

vi.mock("soloclaw/plugin-sdk/browser-node-runtime", () => ({
  withTimeout: vi.fn(
    async (
...
vi.mock("soloclaw/plugin-sdk/browser-setup-tools", () => ({
...
describe("runBrowserProxyCommand", () => {
  ...
  it("rejects unauthorized query.profile when allowProfiles is configured", async () => {
    configMocks.loadConfig.mockReturnValue({
      browser: {},
      nodeHost: { browserProxy: { enabled: true, allowProfiles: ["soloclaw"] } },
    });
    ...
    await expect(
      runBrowserProxyCommand(
        JSON.stringify({
          method: "POST",
          path: "/reset-profile",
          body: { profile: "soloclaw", name: "soloclaw" },
          timeoutMs: 50,
        }),
      ),
    ).rejects.toThrow("INVALID_REQUEST: browser.proxy cannot mutate persistent browser profiles");
