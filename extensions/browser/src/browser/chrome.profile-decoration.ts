...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".soloclaw-profile-decorated");
}
...
export function decorateOpenClawProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_SOLOCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SOLOCLAW_BROWSER_COLOR).toUpperCase();
