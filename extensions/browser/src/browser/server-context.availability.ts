...
export function createProfileAvailability({
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopSoloClawChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchSoloClawChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      // Browser control service can restart while a loopback SoloClaw browser is still
      ...
      const launched = await launchSoloClawChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopSoloClawChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    await stopSoloClawChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchSoloClawChrome(current.resolved, profile);
