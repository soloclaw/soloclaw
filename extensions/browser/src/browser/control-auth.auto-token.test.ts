...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => SoloClawConfig>(),
  writeConfigFile: vi.fn<(cfg: SoloClawConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<SoloClawConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: SoloClawConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): SoloClawConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: SoloClawConfig) {
