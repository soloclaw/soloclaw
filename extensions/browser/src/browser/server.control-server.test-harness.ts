...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.SOLOCLAW_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "soloclaw",
): void {
...
vi.mock("./chrome.js", () => ({
  ...
  launchSoloClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "soloclaw";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.SOLOCLAW_GATEWAY_PORT;
  process.env.SOLOCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.SOLOCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.SOLOCLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SOLOCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.SOLOCLAW_GATEWAY_PASSWORD;
  } else {
    process.env.SOLOCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
