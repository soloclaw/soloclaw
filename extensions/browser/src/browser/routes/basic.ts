...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    if (driver && driver !== "soloclaw" && driver !== "clawd" && driver !== "existing-session") {
      return jsonError(
        res,
        400,
        `unsupported profile driver "${driver}"; use "soloclaw", "clawd", or "existing-session"`,
      );
    }
