...
const { stopSoloClawChromeMock } = vi.hoisted(() => ({
  stopSoloClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopSoloClawChrome: stopSoloClawChromeMock,
}));
...
beforeEach(() => {
  ...
  stopSoloClawChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    ...
    expect(stopMap.soloclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopSoloClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("soloclaw browser stop failed: Error: oops");
  });
});
