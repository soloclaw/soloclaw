...
function localSoloClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "soloclaw",
...
function createLocalSoloClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localSoloClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveSoloClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    ...
    const profileDir = path.join(tempRoot, "soloclaw");
    ...
    const ops = createLocalSoloClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveSoloClawUserDataDir: () => profileDir,
    });
