...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSoloClawTmpDir: vi.fn(() => "/tmp/openclaw"),
}));
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    vi.spyOn(tmpDirModule, "resolvePreferredSoloClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredSoloClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSoloClawTmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSoloClawTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredSoloClawTmpDir).toHaveBeenCalled();
  });
