...
type StopChromeTarget = Parameters<typeof stopSoloClawChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateSoloClawProfile(userDataDir, { color: DEFAULT_SOLOCLAW_BROWSER_COLOR });
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".soloclaw-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateSoloClawProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
...
describe("browser chrome helpers", () => {
  ...
  it("stopSoloClawChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopSoloClawChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopSoloClawChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
