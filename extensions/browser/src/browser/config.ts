...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_SOLOCLAW_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SOLOCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SOLOCLAW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_SOLOCLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_SOLOCLAW_BROWSER_PROFILE_NAME
        : "user");
