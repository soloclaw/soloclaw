...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds SOLOCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `soloclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service SOLOCLAW_GATEWAY_TOKEN does not match gateway.auth.token in soloclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.SOLOCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.SOLOCLAW_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `soloclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
