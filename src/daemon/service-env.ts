...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.SOLOCLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SOLOCLAW_PROFILE: profile,
    SOLOCLAW_GATEWAY_PORT: String(port),
    SOLOCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SOLOCLAW_SYSTEMD_UNIT: systemdUnit,
    SOLOCLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    SOLOCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SOLOCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    SOLOCLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.SOLOCLAW_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SOLOCLAW_GATEWAY_TOKEN: gatewayToken,
    SOLOCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    SOLOCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    SOLOCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    SOLOCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    SOLOCLAW_LOG_PREFIX: "node",
    SOLOCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    SOLOCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    SOLOCLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    SOLOCLAW_STATE_DIR: sharedEnv.stateDir,
    SOLOCLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.SOLOCLAW_STATE_DIR;
  const configPath = env.SOLOCLAW_CONFIG_PATH;
