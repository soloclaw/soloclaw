...
describe("buildServiceEnvironment", () => {
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SOLOCLAW_PROFILE: "work" },
      port: 18789,
    });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through SOLOCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", SOLOCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.SOLOCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits SOLOCLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".soloclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SOLOCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".soloclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SOLOCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".soloclaw"));
  });

  it("uses SOLOCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SOLOCLAW_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in SOLOCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SOLOCLAW_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SOLOCLAW_STATE_DIR: "C:\\State\\openclaw" };
