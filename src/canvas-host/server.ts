...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.soloclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.soloclawCanvasA2UIAction &&
        typeof window.soloclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.soloclawSendUserAction === "function";
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SoloClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.soloclawSendUserAction === "function"
        ? window.soloclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SOLOCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SOLOCLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SoloClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
