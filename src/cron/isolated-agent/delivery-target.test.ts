      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: SoloClawConfig }) =>
...
function makeCfg(overrides?: Partial<SoloClawConfig>): SoloClawConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeTelegramBoundCfg(accountId = "account-b"): SoloClawConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: SoloClawConfig) {
