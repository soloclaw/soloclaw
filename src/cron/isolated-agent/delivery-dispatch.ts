...
function pruneCompletedDirectCronDeliveries(now: number) {
  const ttlMs = process.env.SOLOCLAW_TEST_FAST === "1" ? 60_000 : 24 * 60 * 60 * 1000;
...
function resolveDirectCronRetryDelaysMs(): readonly number[] {
  return process.env.NODE_ENV === "test" && process.env.SOLOCLAW_TEST_FAST === "1"
    ? [0, 0, 0]
    : [5_000, 10_000, 20_000];
}
    ...
    try {
      ...
      const runDelivery = async () =>
        await deliverOutboundPayloads({
          ...
          // See: https://github.com/soloclaw/soloclaw/issues/40545
          skipQueue: true,
        });
