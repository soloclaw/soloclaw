...
homepage: https://docs.soloclaw.ai/automation/hooks#command-logger
metadata:
  {
    "soloclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.soloclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.soloclaw/logs/commands.log ~/.soloclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openclaw`:
   ```
   /home/username/.soloclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
