...
describe("resolveSoloClawMetadata", () => {
  it("extracts openclaw metadata from parsed frontmatter", () => {
    ...
    const result = resolveSoloClawMetadata(frontmatter);
    ...
    const frontmatter = {
      metadata: JSON.stringify({
        openclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with SoloClaw" },
    ...
    "soloclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with SoloClaw" }],
      },
  }
---
...
`;
    ...
    const openclaw = resolveSoloClawMetadata(frontmatter);
