...
export function buildConfigureCandidates(config: SoloClawConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredSoloClawConfig?: SoloClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredSoloClawConfig ?? params.config;
