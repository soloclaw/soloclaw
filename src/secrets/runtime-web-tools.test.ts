...
function setConfiguredFetchProviderKey(configTarget: SoloClawConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: SoloClawConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: SoloClawConfig, provider: ProviderUnderTest): unknown {
