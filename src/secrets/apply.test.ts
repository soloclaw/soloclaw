...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".soloclaw");
  return {
    rootDir,
    stateDir,
    configPath: path.join(stateDir, "soloclaw.json"),
...
async function createApplyFixture(): Promise<ApplyFixture> {
  ...
  return {
    ...paths,
    env: {
      SOLOCLAW_STATE_DIR: paths.stateDir,
      SOLOCLAW_CONFIG_PATH: paths.configPath,
