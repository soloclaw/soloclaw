...
function getSecretProviders(config: SoloClawConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: SoloClawConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: SoloClawConfig): Array<{ value: SecretRefSource; label: string }> {
...
function resolveConfigureAgentId(config: SoloClawConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: SoloClawConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredSoloClawConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "soloclaw.json",
