...
function collectConfigEnvVarsByTarget(cfg?: SoloClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SoloClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: SoloClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: SoloClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SOLOCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
