...
function expectSoloClawSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = SoloClawSchema.safeParse(params.config);
  expect(res.success).toBe(false);
  if (!res.success) {
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectSoloClawSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = SoloClawSchema.safeParse(config);
    expect(res.success).toBe(true);
    if (res.success) {
