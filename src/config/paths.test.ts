...
describe("oauth paths", () => {
  it("prefers SOLOCLAW_OAUTH_DIR over SOLOCLAW_STATE_DIR", () => {
    const env = {
      SOLOCLAW_OAUTH_DIR: "/custom/oauth",
      SOLOCLAW_STATE_DIR: "/custom/state",
  ...
  it("derives oauth path from SOLOCLAW_STATE_DIR when unset", () => {
    const env = {
      SOLOCLAW_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("gateway port resolution", () => {
  it("prefers numeric env values over config", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19002 } }, envWith({ SOLOCLAW_GATEWAY_PORT: "19001" })),
    ).toBe(19001);
  });
  ...
  it("accepts Compose-style IPv4 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ SOLOCLAW_GATEWAY_PORT: "127.0.0.1:18789" }),
      ),
    ).toBe(18789);
  });
  ...
  it("accepts Compose-style IPv6 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ SOLOCLAW_GATEWAY_PORT: "[::1]:28789" }),
      ),
    ).toBe(28789);
  });
  ...
  it("falls back to config when the Compose-style suffix is invalid", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19003 } },
        envWith({ SOLOCLAW_GATEWAY_PORT: "127.0.0.1:not-a-port" }),
      ),
    ).toBe(19003);
  });
  ...
  it("falls back when malformed IPv6 inputs do not provide an explicit port", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ SOLOCLAW_GATEWAY_PORT: "::1" })),
    ).toBe(19003);
    expect(resolveGatewayPort({}, envWith({ SOLOCLAW_GATEWAY_PORT: "2001:db8::1" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });

  it("falls back to the default port when env is invalid and config is unset", () => {
    expect(resolveGatewayPort({}, envWith({ SOLOCLAW_GATEWAY_PORT: "127.0.0.1:not-a-port" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });
});
...
describe("state + config path candidates", () => {
  ...
  it("uses SOLOCLAW_STATE_DIR when set", () => {
    const env = {
      SOLOCLAW_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;

    expect(resolveStateDir(env, () => "/home/test")).toBe(path.resolve("/new/state"));
  });

  it("uses SOLOCLAW_HOME for default state/config locations", () => {
    const env = {
  ...
  it("prefers ~/.soloclaw when it exists and legacy dir is missing", async () => {
    await withTempDir({ prefix: "openclaw-state-" }, async (root) => {
      const newDir = path.join(root, ".soloclaw");
  ...
  it("falls back to existing legacy state dir when ~/.soloclaw is missing", async () => {
