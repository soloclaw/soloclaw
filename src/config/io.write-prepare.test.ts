...
describe("config io write prepare", () => {
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("soloclaw config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('soloclaw config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/soloclaw.json",
        expectedConfigPath: "/tmp/soloclaw.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: SoloClawConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("preserves normalized nested channel enabled keys during unrelated writes", () => {
    ...
    const nextConfig: SoloClawConfig = {
