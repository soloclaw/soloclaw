...
describe("config io paths", () => {
  ...
  it("uses SOLOCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { SOLOCLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
  ...
  it("honors explicit SOLOCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".soloclaw", 20002, "custom.json");
      const io = createIoForHome(home, { SOLOCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
