...
export function parseSoloClawVersion(raw: string | null | undefined): SoloClawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeSoloClawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseSoloClawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameSoloClawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseSoloClawVersion(a);
  const parsedB = parseSoloClawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareSoloClawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseSoloClawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseSoloClawVersion(current);
  const parsedTouched = parseSoloClawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameSoloClawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareSoloClawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: SoloClawVersion): number {
