...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SOLOCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from SOLOCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SOLOCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SOLOCLAW_STATE_DIR differs from stored paths", () => {
