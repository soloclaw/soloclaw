...
// Keep integration tests deterministic: never read a real soloclaw.json.
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.SOLOCLAW_SESSION_CACHE_TTL_MS;
    process.env.SOLOCLAW_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SOLOCLAW_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SOLOCLAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
