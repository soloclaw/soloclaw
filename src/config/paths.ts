...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.SOLOCLAW_NIX_MODE === "1";
}
...
/** Build a homedir thunk that respects SOLOCLAW_HOME for the given env. */
 ...
 * Can be overridden via SOLOCLAW_STATE_DIR.
 * Default: ~/.soloclaw
 */
export function resolveStateDir(
  ...
  const override = env.SOLOCLAW_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.SOLOCLAW_TEST_FAST === "1") {
    return newDir;
  }
 ...
 * Can be overridden via SOLOCLAW_CONFIG_PATH.
 * Default: ~/.soloclaw/soloclaw.json (or $SOLOCLAW_STATE_DIR/soloclaw.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.SOLOCLAW_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.SOLOCLAW_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.SOLOCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const openclawStateDir = env.SOLOCLAW_STATE_DIR?.trim();
  if (openclawStateDir) {
 ...
 *
 * Precedence:
 * - `SOLOCLAW_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.SOLOCLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
