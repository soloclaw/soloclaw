...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.SOLOCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    SOLOCLAW_STATE_DIR: path.join(os.tmpdir(), "openclaw-config-doc-baseline-state"),
    SOLOCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
