...
import { buildWebSearchProviderConfig, withTempHome, writeSoloClawConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = SoloClawSchema.safeParse({
  ...
  it("rejects non-string $schema", () => {
    const result = SoloClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = SoloClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("accepts top-level memorySearch via auto-migration and reports legacyIssues", async () => {
    await withTempHome(async (home) => {
      await writeSoloClawConfig(home, {
        memorySearch: {
          provider: "local",
          fallback: "none",
          query: { maxResults: 7 },
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeSoloClawConfig(home, {
        gateway: { bind: "${SOLOCLAW_BIND}" },
      });

      const prev = process.env.SOLOCLAW_BIND;
      process.env.SOLOCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.SOLOCLAW_BIND;
        } else {
          process.env.SOLOCLAW_BIND = prev;
        }
      }
    });
  });
