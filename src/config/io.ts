...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SOLOCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SOLOCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `SOLOCLAW_CONFIG_PATH` (and friends) are expected to work even
