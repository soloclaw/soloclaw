...
import { createSoloClawCodingTools } from "../../agents/pi-tools.js";
import { resolveSandboxRuntimeStatus } from "../../agents/sandbox.js";
import { buildAgentSystemPrompt } from "../../agents/system-prompt.js";
import { resolveCommandsSystemPromptBundle } from "./commands-system-prompt.js";
import type { HandleCommandsParams } from "./commands-types.js";

const { createSoloClawCodingToolsMock } = vi.hoisted(() => ({
  createSoloClawCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createSoloClawCodingTools: createSoloClawCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createSoloClawCodingToolsMock.mockClear();
    createSoloClawCodingToolsMock.mockReturnValue([]);
  });

  it("opts command tool builds into gateway subagent binding", async () => {
    await resolveCommandsSystemPromptBundle(makeParams());

    expect(vi.mocked(createSoloClawCodingTools)).toHaveBeenCalledWith(
