...
export function resolveAcpInstallCommandHint(cfg: SoloClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `soloclaw plugins install ${workspaceLocalPath}`;
    }
    ...
    return "soloclaw plugins install acpx";
  }
