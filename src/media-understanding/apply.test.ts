...
function createGroqAudioConfig(): SoloClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SoloClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SoloClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SoloClawConfig = { tools: { media: { audio: {} } } };
