...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("soloclaw gateway run")}`,
          `Or rerun with: ${formatCliCommand("soloclaw onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("soloclaw onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $SOLOCLAW_CONFIG_PATH (default: ~/.soloclaw/soloclaw.json) under gateway.auth.token, or in SOLOCLAW_GATEWAY_TOKEN.",
        ...
        `Open the dashboard anytime: ${formatCliCommand("soloclaw dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control SoloClaw."
            : "Copy/paste this URL in a browser on this machine to control SoloClaw.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("soloclaw dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.soloclaw.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.soloclaw.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("soloclaw configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("soloclaw configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.soloclaw.ai/tools/web"}`,
          "Docs: https://docs.soloclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("soloclaw configure --section web")}`,
          "",
          "Docs: https://docs.soloclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control SoloClaw."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control SoloClaw.",
  );
