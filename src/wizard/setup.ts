...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "If you’re not comfortable with security hardening and access control, don’t run SoloClaw.",
      ...
      "soloclaw security audit --fix",
      "",
      "Must read: https://docs.soloclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  let baseConfig: SoloClawConfig = snapshot.valid
    ? snapshot.exists
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("soloclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("soloclaw doctor")}`,
        `Inspect: ${formatCliCommand("soloclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("soloclaw configure")}.`;
  ...
  let localGatewayToken = process.env.SOLOCLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.SOLOCLAW_GATEWAY_PASSWORD;
  try {
