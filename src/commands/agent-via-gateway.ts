...
function parseTimeoutSeconds(opts: { cfg: SoloClawConfig; timeout?: string }) {
  const raw =
...
export async function agentViaGatewayCommand(opts: AgentCliOpts, runtime: RuntimeEnv) {
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("soloclaw agents list")}" to see configured agents.`,
      );
    }
  }
