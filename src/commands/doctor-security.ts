...
import type { SoloClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: SoloClawConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: SoloClawConfig): string[] {
  const warnings: string[] = [];
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("soloclaw approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: SoloClawConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: SoloClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("soloclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.soloclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("soloclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("soloclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("soloclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("soloclaw doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "soloclaw config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("soloclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('soloclaw config set session.dmScope "per-channel-peer"') +
