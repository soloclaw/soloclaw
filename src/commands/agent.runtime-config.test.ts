...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: SoloClawConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => SoloClawConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: SoloClawConfig, sourceConfig: SoloClawConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      ...
      effectiveConfig: SoloClawConfig;
      diagnostics: never[];
    }>
  >(),
);
...
function mockConfig(home: string, storePath: string): SoloClawConfig {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteMock.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as SoloClawConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand runtime config", () => {
  ...
  it("includes channel secret targets when delivery is requested", async () => {
    await withTempHome(async (home) => {
      ...
      } as unknown as SoloClawConfig["channels"];
      resolveCommandConfigWithSecretsMock.mockResolvedValueOnce({
