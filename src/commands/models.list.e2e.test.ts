...
const ensureSoloClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveSoloClawAgentDir = vi.fn().mockReturnValue("/tmp/openclaw-agent");
...
vi.mock("./models/list.runtime.js", () => {
  ...
  return {
    ...
    ensureSoloClawModelsJson,
    resolveSoloClawAgentDir,
...
beforeEach(() => {
  ...
  ensureSoloClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureSoloClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureSoloClawModelsJson).toHaveBeenCalled();
    expect(ensureSoloClawModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
