...
beforeAll(() => {
  envSnapshot = captureEnv(["SOLOCLAW_PROFILE"]);
  process.env.SOLOCLAW_PROFILE = "isolated";
});
...
async function createMockStatusScanResult(params: { includePluginCompatibility?: boolean } = {}) {
  ...
  return {
    ...
    gatewayProbeAuth: process.env.SOLOCLAW_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.soloclaw.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.soloclaw.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.SOLOCLAW_GATEWAY_TOKEN?.trim();
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("SOLOCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
