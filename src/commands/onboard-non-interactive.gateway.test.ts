...
function resolveTestConfigPath() {
  const override = process.env.SOLOCLAW_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.SOLOCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("SOLOCLAW_STATE_DIR must be set before config IO in this test");
  }
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `soloclaw gateway status --deep` for more detail.");
