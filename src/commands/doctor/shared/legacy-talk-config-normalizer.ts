...
export function normalizeLegacyTalkConfig(cfg: SoloClawConfig, changes: string[]): SoloClawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as SoloClawConfig["talk"]) ?? {};
