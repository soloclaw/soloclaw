...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as SoloClawConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as SoloClawConfig["channels"],
  };
}
...
>;
type ModelsConfigPatch = Partial<NonNullable<SoloClawConfig["models"]>>;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<SoloClawConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as SoloClawConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<SoloClawConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyMistralModelMaxTokens(
  ...
  return {
    ...
    models: {
      ...cfg.models,
      providers: nextProviders as NonNullable<SoloClawConfig["models"]>["providers"],
    },
  };
}
