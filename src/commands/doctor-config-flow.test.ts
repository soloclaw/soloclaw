    ...
    findLegacyConfigIssues: (raw: unknown, sourceRaw?: unknown, extraRules: LegacyRule[] = []) => {
      ...
      if ("heartbeat" in root) {
        addIssue(
          issues,
          ["heartbeat"],
          'heartbeat is legacy; use agents.defaults.heartbeat and channels.defaults.heartbeat. Run "soloclaw doctor --fix".',
        );
      }
      if ("memorySearch" in root) {
        addIssue(
          issues,
          ["memorySearch"],
          'memorySearch is legacy; use agents.defaults.memorySearch. Run "soloclaw doctor --fix".',
        );
      }
      ...
      if (gateway && "bind" in gateway) {
        addIssue(
          issues,
          ["gateway", "bind"],
          'gateway.bind host aliases are legacy; use the canonical bind mode. Run "soloclaw doctor --fix".',
        );
      }
      ...
      if (sessionThreadBindings && "ttlHours" in sessionThreadBindings) {
        addIssue(
          ...
          'session.threadBindings.ttlHours is legacy; use session.threadBindings.idleHours. Run "soloclaw doctor --fix".',
        );
      }
      ...
      if (xSearch && "apiKey" in xSearch) {
        addIssue(
          ...
          'tools.web.x_search.apiKey is legacy; use plugins.entries.xai.config.webSearch.apiKey. Run "soloclaw doctor --fix".',
        );
      }
      ...
      if (sandbox && "perSession" in sandbox) {
        addIssue(
          ...
          'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope. Run "soloclaw doctor --fix".',
        );
      }
      ...
      for (const [channelId, channelRaw] of Object.entries(channels ?? {})) {
        ...
        if (hasLegacyStreamingAlias(channel)) {
          addIssue(
            ...
            channelId === "googlechat"
              ? `channels.${channelId}.streamMode is legacy and no longer used. Run "soloclaw doctor --fix".`
              : `channels.${channelId}.streamMode, channels.${channelId}.streaming aliases are legacy. Run "soloclaw doctor --fix".`,
          );
        }
        ...
        if (threadBindings && "ttlHours" in threadBindings) {
          addIssue(
            ...
            'channels.<id>.threadBindings.ttlHours is legacy; use channels.<id>.threadBindings.idleHours. Run "soloclaw doctor --fix".',
          );
        }
        if (channelId === "slack") {
          for (const roomRaw of Object.values(asRecord(channel.channels) ?? {})) {
            if ("allow" in (asRecord(roomRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "slack"],
                'channels.slack.channels.<id>.allow is legacy; use enabled. Run "soloclaw doctor --fix".',
              );
            }
          }
        }
          ...
          for (const spaceRaw of Object.values(asRecord(channel.groups) ?? {})) {
            if ("allow" in (asRecord(spaceRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "googlechat"],
                'channels.googlechat.groups.<id>.allow is legacy; use enabled. Run "soloclaw doctor --fix".',
              );
            }
          }
        }
            ...
            for (const roomRaw of Object.values(asRecord(guild?.channels) ?? {})) {
              if ("allow" in (asRecord(roomRaw) ?? {})) {
                addIssue(
                  issues,
                  ["channels", "discord"],
                  'channels.discord.guilds.<id>.channels.<id>.allow is legacy; use enabled. Run "soloclaw doctor --fix".',
                );
              }
            }
          }
    ...
    listPluginDoctorLegacyConfigRules: () => [
      {
        ...
        message:
          'channels.telegram.groupMentionsOnly was removed; use channels.telegram.groups."*".requireMention instead. Run "soloclaw doctor --fix".',
      },
      {
        path: ["talk"],
        message:
          "talk.voiceId/talk.voiceAliases/talk.modelId/talk.outputFormat/talk.apiKey are legacy; use talk.providers.<provider> and run soloclaw doctor --fix.",
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            message.includes('Run "soloclaw doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockClear();
