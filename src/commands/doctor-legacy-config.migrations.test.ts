...
vi.mock("../plugins/setup-registry.js", () => ({
  runPluginSetupConfigMigrations: ({ config }: { config: SoloClawConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: SoloClawConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    previousOauthDir = process.env.SOLOCLAW_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclaw-oauth-"));
    process.env.SOLOCLAW_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterAll(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.SOLOCLAW_OAUTH_DIR = previousOauthDir;
    }
    fs.rmSync(tempOauthDir, { recursive: true, force: true });
  });
