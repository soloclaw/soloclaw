...
export async function loadValidConfigOrThrow(): Promise<SoloClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: SoloClawConfig) => SoloClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SoloClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SoloClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("soloclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
