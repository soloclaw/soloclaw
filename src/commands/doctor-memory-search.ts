...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("soloclaw doctor --fix")} or ${formatCliCommand("soloclaw memory status --fix")}`
    : `Verify: ${formatCliCommand("soloclaw memory status --deep")}`;
  return [
 ...
 * Runs as part of `soloclaw doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("soloclaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("soloclaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("soloclaw memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`soloclaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("soloclaw configure --section model")}`,
        `- To disable: ${formatCliCommand("soloclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("soloclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
