...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.SOLOCLAW_STATE_DIR ?? "/tmp/openclaw-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.SOLOCLAW_AGENT_DIR ?? "/tmp/openclaw-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under SOLOCLAW_AGENT_DIR when set", async () => {
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.SOLOCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("syncs siblings from explicit agentDir outside SOLOCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-external-"));
    process.env.SOLOCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* SOLOCLAW_STATE_DIR
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to SOLOCLAW_AGENT_DIR when set", async () => {
