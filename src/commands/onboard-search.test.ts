...
function getWebSearchConfig(config: SoloClawConfig | undefined, pluginId: string) {
  return (config as WebSearchConfigRecord | undefined)?.plugins?.entries?.[pluginId]?.config
    ?.webSearch;
}

function ensureWebSearchConfig(config: SoloClawConfig, pluginId: string) {
...
function createSearchProviderEntry(id: string): PluginWebSearchProviderEntry {
  ...
  const entry: PluginWebSearchProviderEntry = {
    ...
    applySelectionConfig: (config) => {
      const next: SoloClawConfig = { ...config, plugins: { ...config.plugins } };
      const entries = { ...next.plugins?.entries } as NonNullable<
        NonNullable<SoloClawConfig["plugins"]>["entries"]
      >;
...
function createPerplexityConfig(apiKey: string, enabled?: boolean): SoloClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: SoloClawConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): SoloClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: SoloClawConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: SoloClawConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
