...
export async function promptGatewayConfig(
  ...
  if (authMode === "token") {
    ...
    if (tokenInputMode === "ref") {
      ...
      note(`Validated ${envVarName}. SoloClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: SoloClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.soloclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
