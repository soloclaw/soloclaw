...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: SoloClawConfig }) =>
      Boolean(config.auth?.profiles?.["openai-codex:default"]),
    ),
    setupChannels: vi.fn(async (cfg: SoloClawConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.soloclaw/soloclaw.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.soloclaw/workspace",
  applyWizardMetadata: (cfg: SoloClawConfig) => cfg,
...
function setupBaseWizardState(config: SoloClawConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: SoloClawConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: SoloClawConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    const baseConfig: SoloClawConfig = {
      plugins: {
        entries: {
          "github-copilot": {
            enabled: false,
    ...
    expect(retryCall.nextConfig).toMatchObject({
      agents: {
        defaults: {
          workspace: expect.stringContaining("/.soloclaw/workspace"),
        },
      },
      plugins: {
        entries: {
