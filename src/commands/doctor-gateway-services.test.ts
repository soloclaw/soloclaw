...
const originalUpdateInProgress = process.env.SOLOCLAW_UPDATE_IN_PROGRESS;
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      SOLOCLAW_GATEWAY_TOKEN: "stale-token",
    },
  });
  mocks.auditGatewayServiceConfig.mockResolvedValue({
    ok: false,
    issues: [
      {
        ...
        message: "Gateway service SOLOCLAW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  afterEach(() => {
    ...
    if (originalUpdateInProgress === undefined) {
      ...
      process.env.SOLOCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
    }
  });
  ...
  it("uses SOLOCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            SOLOCLAW_GATEWAY_TOKEN: "env-file-token",
          },
          environmentValueSources: {
            SOLOCLAW_GATEWAY_TOKEN: "file",
          },
        });
        mocks.auditGatewayServiceConfig.mockResolvedValue({
          ok: false,
