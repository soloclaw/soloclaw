...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SOLOCLAW_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: SoloClawConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: SoloClawConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: SoloClawConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: SoloClawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SOLOCLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<SoloClawConfig> {
