...
vi.mock("../channels/plugins/bundled.js", async () => {
  ...
  function resolveTelegramAccountId(cfg: SoloClawConfig): string {
  ...
  function detectTelegramAllowFromMigration(params: {
    ...
    const root = params.env.SOLOCLAW_STATE_DIR;
    if (!root) {
      return [];
    }
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: SoloClawConfig; env: NodeJS.ProcessEnv }) =>
...
async function runTelegramAllowFromMigration(params: { root: string; cfg: SoloClawConfig }) {
  const oauthDir = ensureCredentialsDir(params.root);
  writeLegacyTelegramAllowFromStore(oauthDir);
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { SOLOCLAW_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
function getStateDirMigrationPaths(root: string) {
  return {
    targetDir: path.join(root, ".soloclaw"),
...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    const detected = await detectLegacyStateMigrations({
      cfg,
      env: { SOLOCLAW_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    const cfg: SoloClawConfig = { session: { mainKey: "work" } };
    writeLegacySessionsFixture({
      root,
      sessions: {
        "+1555": { sessionId: "a", updatedAt: 10 },
