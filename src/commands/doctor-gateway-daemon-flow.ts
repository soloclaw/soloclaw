...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.SOLOCLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("soloclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
