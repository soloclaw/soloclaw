...
function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ...
    : `${POSIX_SOLOCLAW_TMP_DIR}/openclaw.log`;
}
...
function resolveSettings(): ResolvedSettings {
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.SOLOCLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[soloclaw] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
