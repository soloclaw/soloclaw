...
export const SOLOCLAW_NODE_OPTIONS_READY = "SOLOCLAW_NODE_OPTIONS_READY";
export const SOLOCLAW_NODE_EXTRA_CA_CERTS_READY = "SOLOCLAW_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.SOLOCLAW_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[SOLOCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[SOLOCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[SOLOCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[SOLOCLAW_NODE_OPTIONS_READY] = "1";
