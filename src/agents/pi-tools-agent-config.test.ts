...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SoloClawConfig) {
    return createSoloClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SoloClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SoloClawConfig["agents"]>["list"]>[number]["tools"];
  }): SoloClawConfig {
    return {
      tools: params.tools,
      agents: {
