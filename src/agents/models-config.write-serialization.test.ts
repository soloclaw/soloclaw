...
const planSoloClawModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planSoloClawModelsJson: (...args: unknown[]) => planSoloClawModelsJsonMock(...args),
  }));
  ({ ensureSoloClawModelsJson } = await import("./models-config.js"));
});

beforeEach(() => {
  planSoloClawModelsJsonMock
    .mockReset()
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureSoloClawModelsJson(first), ensureSoloClawModelsJson(second)]);
      } finally {
