...
describe("buildDockerExecArgs", () => {
  it("prepends custom PATH after login shell sourcing to preserve both custom and system tools", () => {
    ...
    expect(args).toContain("SOLOCLAW_PREPEND_PATH=/custom/bin:/usr/local/bin:/usr/bin");
    expect(commandArg).toContain('export PATH="${SOLOCLAW_PREPEND_PATH}:$PATH"');
    expect(commandArg).toContain("echo hello");
    expect(commandArg).toBe(
      'export PATH="${SOLOCLAW_PREPEND_PATH}:$PATH"; unset SOLOCLAW_PREPEND_PATH; echo hello',
    );
  });
  ...
  it("does not interpolate PATH into the shell command", () => {
    ...
    const commandArg = args[args.length - 1];
    expect(args).toContain(`SOLOCLAW_PREPEND_PATH=${injectedPath}`);
    expect(commandArg).not.toContain(injectedPath);
    expect(commandArg).toContain("SOLOCLAW_PREPEND_PATH");
  });
