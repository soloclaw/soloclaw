...
beforeAll(async () => {
  ...
  await fs.mkdir(path.join(tempHome.home, ".soloclaw", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native SoloClaw slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const config = {
      plugins: {
        entries: {
    ...
    const pluginRoot = path.join(workspaceDir, ".soloclaw", "extensions", "compound-bundle");
