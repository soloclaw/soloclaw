...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SoloClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SoloClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SoloClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: SoloClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: SoloClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SoloClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: SoloClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses SOLOCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("SOLOCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as SoloClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".soloclaw", "workspace"));
  });

  it("uses SOLOCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("SOLOCLAW_HOME", home);
    // Clear state dir so it falls back to SOLOCLAW_HOME
    vi.stubEnv("SOLOCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as SoloClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".soloclaw", "agents", "main", "agent"));
  });
