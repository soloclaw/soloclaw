...
function buildOllamaProviderCfg(apiKey: string): SoloClawConfig {
  return {
    models: {
      providers: {
        ollama: {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    ...
    try {
      ...
      await withEnvAsync(
        {
          SOLOCLAW_STATE_DIR: tempDir,
          SOLOCLAW_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.SOLOCLAW_AGENT_DIR, {
