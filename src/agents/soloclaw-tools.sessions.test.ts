...
import "./test-helpers/fast-soloclaw-tools-sessions.js";
...
function createSoloClawTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    const tools = createSoloClawTools();
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createSoloClawTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createSoloClawTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
