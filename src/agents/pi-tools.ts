...
import { createSoloClawTools } from "./soloclaw-tools.js";
...
function resolveExecConfig(params: { cfg?: SoloClawConfig; agentId?: string }) {
...
export function createSoloClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createSoloClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
