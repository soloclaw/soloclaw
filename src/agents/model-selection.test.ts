...
function resolveAnthropicOpusThinking(cfg: SoloClawConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: SoloClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<SoloClawConfig>) {
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as SoloClawConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as SoloClawConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<SoloClawConfig> = {
        agents: {
          defaults: {
            models: {
