...
describe("resolveSoloClawAgentDir", () => {
      ...
      withEnv(
        {
          ...
          SOLOCLAW_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSoloClawAgentDir();
  ...
  it("honors SOLOCLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          SOLOCLAW_STATE_DIR: undefined,
          SOLOCLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSoloClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when SOLOCLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers SOLOCLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          SOLOCLAW_STATE_DIR: undefined,
          SOLOCLAW_AGENT_DIR: primaryOverride,
