...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  ...
  return path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-system-prompt-"),
  );
