...
describe("resolveBundledSkillsDir", () => {
  ...
  it("returns SOLOCLAW_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-bundled-override-"));
    process.env.SOLOCLAW_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "soloclaw" }));
