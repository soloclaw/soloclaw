...
} from "./soloclaw-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.SOLOCLAW_TEST_FAST;
...
describe("soloclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.SOLOCLAW_TEST_FAST = fastModeEnv.previous;
  });
