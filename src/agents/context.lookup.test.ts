...
vi.mock("./models-config.js", () => ({
  ensureSoloClawModelsJson: contextTestState.ensureSoloClawModelsJson,
}));

vi.mock("./agent-paths.js", () => ({
  resolveSoloClawAgentDir: () => "/tmp/openclaw-agent",
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureSoloClawModelsJson.mockClear();
  return { ensureSoloClawModelsJson: contextTestState.ensureSoloClawModelsJson };
}
  ...
  it("only warms eagerly for real openclaw startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        ...
        const { ensureSoloClawModelsJson } = mockContextModuleDeps(loadConfigMock);
        ...
        expect(ensureSoloClawModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
