...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.soloclaw-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      '[embed url="/__soloclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
      "Never use local filesystem paths or `file://...` URLs in `[embed ...]`.",
    );
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.soloclaw-dev/canvas`.",
    );
