...
import { planSoloClawModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: SoloClawConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: SoloClawConfig): {
  config: SoloClawConfig;
  sourceConfigForSecrets: SoloClawConfig;
} {
...
export async function ensureSoloClawModelsJson(
  config?: SoloClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveSoloClawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planSoloClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
