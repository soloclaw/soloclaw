...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.SOLOCLAW_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".soloclaw", `workspace-${profile}`);
  }
  return path.join(home, ".soloclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".soloclaw";
