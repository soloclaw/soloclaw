...
async function withOutsideHardlinkInSoloClawTmp<T>(
  params: {
    soloClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.soloClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.soloClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.soloClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.soloClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const soloClawTmpDir = resolvePreferredSoloClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred SoloClaw tmp root",
      media: path.join(soloClawTmpDir, "image.png"),
      expected: path.join(soloClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred SoloClaw tmp root",
      media: pathToFileURL(path.join(soloClawTmpDir, "photo.png")).href,
      expected: path.join(soloClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred SoloClaw tmp root",
      media: path.join(soloClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(soloClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(soloClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked SoloClaw tmp paths escaping tmp root", async () => {
    ...
    if (isPathInside(soloClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(soloClawTmpDir, { recursive: true });
      const symlinkPath = path.join(soloClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked SoloClaw tmp paths to outside files", async () => {
    ...
    await withOutsideHardlinkInSoloClawTmp(
      {
        soloClawTmpDir,
