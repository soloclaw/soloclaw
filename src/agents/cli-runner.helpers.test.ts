...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-write-images-"),
    );
    ...
    try {
      ...
      expect(first.paths[0]).toContain(`${resolvePreferredSoloClawTmpDir()}/openclaw-cli-images/`);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-prompt-image-"),
    );
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-prompt-image-generic-"),
    );
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".soloclaw-cli-images"))).toBe(
        true,
      );
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredSoloClawTmpDir(), "openclaw-cli-explicit-images-"),
    );
