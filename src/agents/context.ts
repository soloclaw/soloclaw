...
function isLikelySoloClawCliProcess(argv: string[] = process.argv): boolean {
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  ...
  // built plugin-sdk can call ensureSoloClawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelySoloClawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): SoloClawConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureSoloClawModelsJson(cfg);
    } catch {
