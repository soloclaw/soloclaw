...
function makeFallbacksOnlyCfg(): SoloClawConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): SoloClawConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
// https://github.com/soloclaw/soloclaw/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal SoloClaw compatibility marker, not a provider API contract.
