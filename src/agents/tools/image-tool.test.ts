...
type CreateSoloClawCodingToolsArgs = Parameters<typeof createSoloClawCodingTools>[0];
type MockSoloClawToolsOptions = {
...
vi.mock("../soloclaw-tools.js", async () => {
  const { createImageTool } = await import("./image-tool.js");
  return {
    createSoloClawTools: vi.fn((options?: MockSoloClawToolsOptions) => {
...
async function createSoloClawCodingToolsWithFreshModules(options?: CreateSoloClawCodingToolsArgs) {
  ...
  return createSoloClawCodingTools(options);
}
...
function createMinimaxImageConfig(): SoloClawConfig {
  return {
    agents: {
      defaults: {
...
describe("image tool implicit imageModel config", () => {
  ...
  it("allows workspace images via createSoloClawCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createSoloClawCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.soloclaw/media/inbound/photo.png",
      });
