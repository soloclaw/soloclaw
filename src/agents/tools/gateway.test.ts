...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.SOLOCLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.openclaw === undefined) {
      ...
      process.env.SOLOCLAW_GATEWAY_TOKEN = envSnapshot.openclaw;
    }
  });
  ...
  it("uses SOLOCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SOLOCLAW_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
