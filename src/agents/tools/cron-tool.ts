...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    label: "Cron",
    name: "cron",
    ownerOnly: isSoloClawOwnerOnlyCoreToolName("cron"),
...
Main-session cron jobs enqueue system events for heartbeat handling. Isolated cron jobs create background task runs that appear in \`soloclaw tasks\`.
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "add": {
          ...
          // See: https://github.com/soloclaw/soloclaw/issues/11310
          if (
            !params.job ||
