...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SOLOCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SOLOCLAW_HOME: "/srv/openclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openclaw-home"), ".soloclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".soloclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".soloclaw"), { recursive: true });
    await fs.writeFile(
