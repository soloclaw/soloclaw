...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.SOLOCLAW_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.SOLOCLAW_AUTH_STORE_READONLY;
        } else {
          process.env.SOLOCLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
  ...
  it("includes env-backed provider auth when no auth profile exists", async () => {
    ...
    const previousDisableBundledPlugins = process.env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS;
