...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<SoloClawConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<SoloClawConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<SoloClawConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<SoloClawConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
