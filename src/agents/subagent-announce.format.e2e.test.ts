...
let configOverride: SoloClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: SoloClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/soloclaw/soloclaw/issues/31298
    previousFastTestEnv = process.env.SOLOCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.SOLOCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // SOLOCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("SoloClaw runtime context (internal):");
