...
const GOOGLE_LIVE_SINGLE_KEY = "SOLOCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SOLOCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "SOLOCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SOLOCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SOLOCLAW_LIVE_${base}_KEY`;
