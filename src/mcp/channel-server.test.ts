...
import { z } from "zod";
import { createSoloClawChannelMcpServer, SoloClawChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createSoloClawChannelMcpServer({
...
describe("soloclaw channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new SoloClawChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
