...
import { z } from "zod";
import type { SoloClawChannelBridge } from "./channel-bridge.js";
import {
...
export function registerChannelMcpTools(server: McpServer, bridge: SoloClawChannelBridge): void {
  server.tool(
    "conversations_list",
    "List SoloClaw channel-backed conversations available through session routes.",
    {
  ...
  server.tool(
    "conversation_get",
    "Get one SoloClaw conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one SoloClaw conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one SoloClaw conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued SoloClaw conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued SoloClaw conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same SoloClaw conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open SoloClaw exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending SoloClaw exec or plugin approval request.",
    {
