...
vi.mock("./channel-bridge.js", () => ({
  SoloClawChannelBridge: class MockSoloClawChannelBridge {
...
describe("serveSoloClawChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveSoloClawChannelMcp } = await import("./channel-server.js");

    const servePromise = serveSoloClawChannelMcp({ verbose: false });
