...
const SOLOCLAW_SOURCE_EXTENSIONS_ROOT = path.resolve(SOLOCLAW_PACKAGE_ROOT, "extensions");
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  if (preferSource) {
    const modulePath =
      resolveBundledPluginSourcePublicSurfacePath({
        ...params,
        sourceRoot: bundledPluginsDir ?? path.resolve(SOLOCLAW_PACKAGE_ROOT, "extensions"),
      }) ??
      resolveBundledPluginPublicSurfacePath({
        rootDir: SOLOCLAW_PACKAGE_ROOT,
        env: params.env,
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: SOLOCLAW_SOURCE_EXTENSIONS_ROOT,
    resolutionKey: createFacadeResolutionKey(params),
  };
}
