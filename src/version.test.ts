...
describe("version resolution", () => {
  ...
  it("prefers SOLOCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        SOLOCLAW_VERSION: "9.9.9",
        SOLOCLAW_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale SOLOCLAW_VERSION for compatibility checks", () => {
    const previous = process.env.SOLOCLAW_VERSION;
    const previousService = process.env.SOLOCLAW_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.SOLOCLAW_VERSION = "2026.3.25";
      process.env.SOLOCLAW_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.SOLOCLAW_VERSION = previous;
      process.env.SOLOCLAW_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        SOLOCLAW_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        SOLOCLAW_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        SOLOCLAW_VERSION: "2026.3.99",
        SOLOCLAW_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        SOLOCLAW_VERSION: "   ",
        SOLOCLAW_SERVICE_VERSION: "  2.0.0  ",
