...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.SOLOCLAW_STATE_DIR ?? "/tmp/openclaw",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.SOLOCLAW_CONFIG_PATH ?? `${stateDir}/soloclaw.json`,
    );
  ...
  it("falls back to SOLOCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers SOLOCLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "SOLOCLAW_GATEWAY_PASSWORD"; // pragma: allowlist secret
