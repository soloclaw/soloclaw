...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run soloclaw devices list",
      pairingHint:
        "Pairing required. Run `soloclaw devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `soloclaw tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
