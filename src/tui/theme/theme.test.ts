...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      SOLOCLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when SOLOCLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ SOLOCLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when SOLOCLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ SOLOCLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats SOLOCLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ SOLOCLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("SOLOCLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      SOLOCLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ SOLOCLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ SOLOCLAW_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.SOLOCLAW_THEME = "light";
