...
describe("music-generation runtime", () => {
  ...
  it("lists runtime music-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeMusicGenerationProviders({ config: {} as SoloClawConfig })).toEqual(
      providers,
    );
    expect(mocks.listMusicGenerationProviders).toHaveBeenCalledWith({} as SoloClawConfig);
  });
