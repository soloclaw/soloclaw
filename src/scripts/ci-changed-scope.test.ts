...
describe("detectChangedScope", () => {
  ...
  it("keeps node lane off for native-only changes", () => {
    ...
    expect(detectChangedScope(["apps/shared/SoloClawKit/Sources/Foo.swift"])).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: true,
      runWindows: false,
  ...
  it("does not force macOS for generated protocol model-only changes", () => {
    expect(detectChangedScope(["apps/macos/Sources/SoloClawProtocol/GatewayModels.swift"])).toEqual(
      {
        runNode: false,
        runMacos: false,
        runAndroid: false,
