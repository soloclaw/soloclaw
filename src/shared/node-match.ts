...
function isCurrentSoloClawClient(clientId: string | undefined): boolean {
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentSoloClawClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentSoloClawClient(node.clientId)) {
    score += 10;
