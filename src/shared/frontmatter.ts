...
export function resolveSoloClawManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type SoloClawManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveSoloClawManifestRequires(
  metadataObj: Record<string, unknown>,
): SoloClawManifestRequires | undefined {
  const requiresRaw =
...
export function resolveSoloClawManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveSoloClawManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedSoloClawManifestInstallBase = {
...
export function parseSoloClawManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedSoloClawManifestInstallBase | undefined {
  ...
  const spec: ParsedSoloClawManifestInstallBase = {
    raw,
    kind,
  };
...
export function applySoloClawManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedSoloClawManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
