...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): SoloClawConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as SoloClawConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: SoloClawConfig) => string[];
  input: SoloClawConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: SoloClawConfig, expected: string) {
