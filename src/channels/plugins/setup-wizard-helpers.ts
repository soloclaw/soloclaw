...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: SoloClawConfig) => Array<string | number> | undefined;
}): SoloClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: SoloClawConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: SoloClawConfig) => Array<string | number> | undefined;
}): (cfg: SoloClawConfig, dmPolicy: DmPolicy) => SoloClawConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: SoloClawConfig, allowFrom: string[]) => SoloClawConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: SoloClawConfig, groupPolicy: "open" | "allowlist" | "disabled") => SoloClawConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: SoloClawConfig) => SoloClawConfig | Promise<SoloClawConfig>;
  applySet?: (
    cfg: SoloClawConfig,
    value: SecretInput,
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in SoloClaw config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "SOLOCLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "SOLOCLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends SoloClawConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends SoloClawConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  channel: string;
  defaultAccountId: string | ((cfg: SoloClawConfig) => string);
  enabled?: boolean;
  noteTitle?: string;
  noteLines?: string[];
  message: string;
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: SoloClawConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
  ...
  const sharedParams = {
    ...
    placeholder: params.placeholder,
    parseEntries: params.parseEntries,
    getExistingAllowFrom: ({ cfg }: { cfg: SoloClawConfig }) =>
      params.getExistingAllowFrom?.(cfg) ??
      (cfg.channels?.[params.channel] as { allowFrom?: Array<string | number> } | undefined)
        ?.allowFrom ??
      [],
    ...(params.mergeEntries ? { mergeEntries: params.mergeEntries } : {}),
    applyAllowFrom: ({ cfg, allowFrom }: { cfg: SoloClawConfig; allowFrom: string[] }) =>
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: SoloClawConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: SoloClawConfig) => Array<string | number>;
