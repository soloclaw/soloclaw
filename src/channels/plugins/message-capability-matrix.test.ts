...
describe("channel action capability matrix", () => {
  ...
  function getCapabilities(plugin: Pick<ChannelPlugin, "actions">, cfg: SoloClawConfig) {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = getCapabilities(telegramPlugin, {} as SoloClawConfig);
    ...
    const discordResult = getCapabilities(discordPlugin, {} as SoloClawConfig);
