...
describe("group policy warning builders", () => {
  ...
  it("projects cfg-only warning collector inputs", () => {
    const collect = projectConfigWarningCollector<{ cfg: SoloClawConfig; accountId: string }>(
      ({ cfg }) => [cfg.channels ? "configured" : "none"],
    );

    expect(
      collect({
        cfg: { channels: { slack: {} } } as SoloClawConfig,
        accountId: "acct-1",
      }),
    ).toEqual(["configured"]);
  });
  ...
  it("projects account+cfg warning collector inputs", () => {
    const collect = projectAccountConfigWarningCollector<
      ...
      { account: { accountId: string }; cfg: SoloClawConfig }
    >(
      (cfg: SoloClawConfig) => cfg.channels ?? {},
