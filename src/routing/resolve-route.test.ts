...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: SoloClawConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: SoloClawConfig };
    expected: ResolvedRouteExpectation;
  }) => {
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<SoloClawConfig["bindings"]>[number];
