...
export async function runSoloClawCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  ...
  const { stdout, stderr } = await execFileAsync(process.execPath, ["soloclaw.mjs", ...args], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: 30_000,
    maxBuffer: 1024 * 1024,
  ...
  if (!trimmed) {
    throw new Error(
      [
        `soloclaw ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `soloclaw ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
