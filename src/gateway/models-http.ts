...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([SOLOCLAW_MODEL_ID, SOLOCLAW_DEFAULT_MODEL_ID]);
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== SOLOCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
