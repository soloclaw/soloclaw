...
const { createSoloClawTools } = await import("../agents/soloclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createSoloClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSoloClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SOLOCLAW_GATEWAY_PORT", "SOLOCLAW_GATEWAY_TOKEN"]);
  ...
  process.env.SOLOCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.SOLOCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("SOLOCLAW_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createSoloClawTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
