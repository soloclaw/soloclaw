...
const connectNodeClient = async (params: {
  ...
  const token = process.env.SOLOCLAW_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("SOLOCLAW_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("SOLOCLAW_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
