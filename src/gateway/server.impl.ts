...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.SOLOCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.SOLOCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "SOLOCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "SOLOCLAW_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `soloclaw config set gateway.auth.mode token` and `soloclaw config set gateway.auth.token <token>`.",
      );
    }
  }
