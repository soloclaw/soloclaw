...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("soloclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.SOLOCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
