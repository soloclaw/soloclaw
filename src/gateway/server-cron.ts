...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.SOLOCLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: SoloClawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: SoloClawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: SoloClawConfig, requestedAgentId: string) => {
