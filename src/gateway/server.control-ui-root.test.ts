...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  ...
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "soloclaw");
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global SoloClaw package control-ui", async () => {
