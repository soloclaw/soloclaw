...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/soloclaw.json" }),
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: SoloClawConfig) => ({
    ok: true,
    config,
  }));
...
describe("config shared auth disconnects", () => {
  it("does not disconnect shared-auth clients for config.set auth writes without restart", async () => {
    const prevConfig: SoloClawConfig = {
      gateway: {
        auth: {
          mode: "token",
          token: "old-token",
