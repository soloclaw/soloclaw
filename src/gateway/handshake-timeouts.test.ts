...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers SOLOCLAW_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        SOLOCLAW_HANDSHAKE_TIMEOUT_MS: "75",
        SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        SOLOCLAW_HANDSHAKE_TIMEOUT_MS: "",
        SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("getConnectChallengeTimeoutMsFromEnv reads SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS;
    try {
      process.env.SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      if (original === undefined) {
        delete process.env.SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.SOLOCLAW_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
    }
  });
});
