...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers SOLOCLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.SOLOCLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.SOLOCLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.SOLOCLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.SOLOCLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.SOLOCLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.SOLOCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            SOLOCLAW_VERSION: " ",
            SOLOCLAW_SERVICE_VERSION: "2.4.6-service",
