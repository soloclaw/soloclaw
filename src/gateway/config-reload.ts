...
export function resolveGatewayReloadSettings(cfg: SoloClawConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: SoloClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: SoloClawConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: SoloClawConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: SoloClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: SoloClawConfig) => {
