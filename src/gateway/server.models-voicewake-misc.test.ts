...
describe("gateway server models + voicewake", () => {
  ...
  const withModelsConfig = async <T>(config: unknown, run: () => Promise<T>): Promise<T> => {
    ...
    if (!configPath) {
      throw new Error("Missing SOLOCLAW_CONFIG_PATH");
    }
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".soloclaw", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    await withEnvAsync({ SOLOCLAW_GATEWAY_TOKEN: "secret" }, async () => {
      testTailnetIPv4.value = "100.64.0.1";
      testState.gatewayBind = "lan";
      const canvasPort = await getFreePort();
      testState.canvasHostPort = canvasPort;
      await withEnvAsync({ SOLOCLAW_CANVAS_HOST_PORT: String(canvasPort) }, async () => {
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    ...
    await withEnvAsync({ SOLOCLAW_TEST_MINIMAL_GATEWAY: undefined }, async () => {
