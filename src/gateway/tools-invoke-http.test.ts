...
let lastCreateSoloClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/soloclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSoloClawToolsContext?.agentTo,
          agentThreadId: lastCreateSoloClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSoloClawTools: (ctx: Record<string, unknown>) => {
      lastCreateSoloClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSoloClawToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateSoloClawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateSoloClawToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSoloClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSoloClawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateSoloClawToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateSoloClawToolsContext?.senderIsOwner).toBe(true);
  });
