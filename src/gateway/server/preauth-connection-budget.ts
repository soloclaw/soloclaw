...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__soloclaw_unknown_client_ip__";

export function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.SOLOCLAW_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.SOLOCLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
