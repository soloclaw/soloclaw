...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses SOLOCLAW_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/openclaw-home"), ".soloclaw", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("archiveSessionTranscripts", () => {
  ...
  beforeAll(() => {
    vi.stubEnv("SOLOCLAW_HOME", tmpDir);
  });
