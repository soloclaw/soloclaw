...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.SOLOCLAW_SKIP_CHANNELS;
  const previousSkipProviders = process.env.SOLOCLAW_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.SOLOCLAW_SKIP_CHANNELS;
    } else {
      process.env.SOLOCLAW_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.SOLOCLAW_SKIP_PROVIDERS;
    } else {
      process.env.SOLOCLAW_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
