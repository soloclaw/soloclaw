...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("SOLOCLAW_CONFIG_PATH is required for gateway config tests");
  }
    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "soloclaw", messages: [{ role: "user", content: "hi" }] },
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "soloclaw", messages: [{ role: "user", content: "hi" }] },
          {
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from SoloClaw.");
      }
