...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    SOLOCLAW_CONFIG_DIR: configDir,
    SOLOCLAW_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SOLOCLAW_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SOLOCLAW_EXTRA_MOUNTS: undefined,
      SOLOCLAW_HOME_VOLUME: "openclaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SOLOCLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SOLOCLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("SOLOCLAW_HOME_VOLUME=openclaw-home"); // pragma: allowlist secret
    ...
    expect(log).toContain("--build-arg SOLOCLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  ...
  it("writes SOLOCLAW_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SOLOCLAW_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SOLOCLAW_TZ=Asia/Shanghai");
  });
  ...
  it("reuses existing config token when SOLOCLAW_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("SOLOCLAW_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when SOLOCLAW_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "SOLOCLAW_GATEWAY_TOKEN=dotenv-token-123\nSOLOCLAW_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("SOLOCLAW_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "SOLOCLAW_GATEWAY_TOKEN=first-token",
        "SOLOCLAW_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("SOLOCLAW_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("SOLOCLAW_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats SOLOCLAW_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SOLOCLAW_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg SOLOCLAW_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg SOLOCLAW_INSTALL_DOCKER_CLI=1");
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        SOLOCLAW_SANDBOX: "1",
        SOLOCLAW_DOCKER_SOCKET: socketPath,
  ...
  it("rejects injected multiline SOLOCLAW_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SOLOCLAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SOLOCLAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SOLOCLAW_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SOLOCLAW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SOLOCLAW_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SOLOCLAW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SOLOCLAW_HOME_VOLUME must match");
  });

  it("rejects SOLOCLAW_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SOLOCLAW_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SOLOCLAW_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/SOLOCLAW_GATEWAY_TOKEN: \$\{SOLOCLAW_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{SOLOCLAW_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
