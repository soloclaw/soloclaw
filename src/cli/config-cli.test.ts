...
/**
 * Test for issue #6070:
 * `soloclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: SoloClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (cfg: SoloClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: SoloClawConfig, config: SoloClawConfig) {
...
function withRuntimeDefaults(resolved: SoloClawConfig): SoloClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-soloclaw.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: SoloClawConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__SOLOCLAW_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-soloclaw.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: SoloClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("soloclaw config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "soloclaw config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/soloclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.soloclaw/soloclaw.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.soloclaw/soloclaw.json");
    });
  });
});
