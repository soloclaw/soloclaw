...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/soloclaw-root")).toEqual([
      path.join("/tmp/soloclaw-root", "dist", "index.js"),
      path.join("/tmp/soloclaw-root", "dist", "index.mjs"),
      path.join("/tmp/soloclaw-root", "dist", "entry.js"),
      path.join("/tmp/soloclaw-root", "dist", "entry.mjs"),
    ]);
  });
});
