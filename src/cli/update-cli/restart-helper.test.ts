...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses SOLOCLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SOLOCLAW_PROFILE: "default",
        SOLOCLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.soloclaw.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.soloclaw.gateway'");
      expect(content).toContain("launchctl bootstrap 'gui/501'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses SOLOCLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        SOLOCLAW_PROFILE: "default",
        SOLOCLAW_LAUNCHD_LABEL: "com.custom.soloclaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.soloclaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses SOLOCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        SOLOCLAW_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.soloclaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        SOLOCLAW_LAUNCHD_LABEL: "ai.soloclaw.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.soloclaw.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
