...
const dotenvState = vi.hoisted(() => {
  ...
  return {
    state,
    loadDotEnv: vi.fn(() => {
      state.profileAtDotenvLoad = process.env.SOLOCLAW_PROFILE;
      state.containerAtDotenvLoad = process.env.SOLOCLAW_CONTAINER;
    }),
  };
});
...
describe("runCli profile env bootstrap", () => {
  const originalProfile = process.env.SOLOCLAW_PROFILE;
  const originalStateDir = process.env.SOLOCLAW_STATE_DIR;
  const originalConfigPath = process.env.SOLOCLAW_CONFIG_PATH;
  const originalContainer = process.env.SOLOCLAW_CONTAINER;
  const originalGatewayPort = process.env.SOLOCLAW_GATEWAY_PORT;
  const originalGatewayUrl = process.env.SOLOCLAW_GATEWAY_URL;
  const originalGatewayToken = process.env.SOLOCLAW_GATEWAY_TOKEN;
  const originalGatewayPassword = process.env.SOLOCLAW_GATEWAY_PASSWORD;

  beforeEach(() => {
    delete process.env.SOLOCLAW_PROFILE;
    delete process.env.SOLOCLAW_STATE_DIR;
    delete process.env.SOLOCLAW_CONFIG_PATH;
    delete process.env.SOLOCLAW_CONTAINER;
  ...
  afterEach(() => {
    if (originalProfile === undefined) {
      ...
      process.env.SOLOCLAW_PROFILE = originalProfile;
    }
    if (originalContainer === undefined) {
      delete process.env.SOLOCLAW_CONTAINER;
    } else {
      process.env.SOLOCLAW_CONTAINER = originalContainer;
    }
    ...
    if (originalConfigPath === undefined) {
      ...
      process.env.SOLOCLAW_CONFIG_PATH = originalConfigPath;
    }
    if (originalGatewayPort === undefined) {
      delete process.env.SOLOCLAW_GATEWAY_PORT;
    } else {
      process.env.SOLOCLAW_GATEWAY_PORT = originalGatewayPort;
    }
    if (originalGatewayUrl === undefined) {
      delete process.env.SOLOCLAW_GATEWAY_URL;
    } else {
      process.env.SOLOCLAW_GATEWAY_URL = originalGatewayUrl;
    }
    ...
    if (originalGatewayPassword === undefined) {
      ...
      process.env.SOLOCLAW_GATEWAY_PASSWORD = originalGatewayPassword;
    }
  });
  ...
  it("applies --profile before dotenv loading", async () => {
    ...
    expect(process.env.SOLOCLAW_PROFILE).toBe("rawdog");
  });
  ...
  it("does not let dotenv change container target resolution", async () => {
    ...
    dotenvState.loadDotEnv.mockImplementationOnce(() => {
      process.env.SOLOCLAW_CONTAINER = "demo";
      dotenvState.state.profileAtDotenvLoad = process.env.SOLOCLAW_PROFILE;
  ...
  it("allows container mode when SOLOCLAW_PROFILE is already set in env", async () => {
  ...
  it.each([
    ...
    ["SOLOCLAW_GATEWAY_TOKEN", "demo-token"],
    ["SOLOCLAW_GATEWAY_PASSWORD", "demo-password"],
  ...
  it("allows container mode when only SOLOCLAW_STATE_DIR is set in env", async () => {
  ...
  it("allows container mode when only SOLOCLAW_CONFIG_PATH is set in env", async () => {
