...
describe("gateway run option collisions", () => {
  ...
  it.each([
    ...
    delete process.env.SOLOCLAW_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.SOLOCLAW_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `soloclaw onboard --mode local` or `soloclaw setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "SOLOCLAW_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
