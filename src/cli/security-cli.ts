...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["soloclaw security audit", "Run a local security audit."],
          ["soloclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["soloclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "soloclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["soloclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["soloclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.soloclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("SoloClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("soloclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("soloclaw security audit --fix")}`));
        if (!fixResult) {
