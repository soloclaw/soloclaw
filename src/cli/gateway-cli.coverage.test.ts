...
describe("gateway-cli coverage", () => {
    ...
    await withEnvOverride(
      {
        ...
        SOLOCLAW_LAUNCHD_LABEL: undefined,
        SOLOCLAW_SYSTEMD_UNIT: undefined,
        INVOCATION_ID: undefined,
        SYSTEMD_EXEC_PID: undefined,
        JOURNAL_STREAM: undefined,
        SOLOCLAW_WINDOWS_TASK_NAME: undefined,
        SOLOCLAW_SERVICE_MARKER: undefined,
        SOLOCLAW_SERVICE_KIND: undefined,
      },
      async () => {
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ SOLOCLAW_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
