...
export function registerCronCli(program: Command) {
  const cron = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "docs.soloclaw.ai/cli/cron")}\n${theme.muted("Upgrade tip:")} run \`soloclaw doctor --fix\` to normalize legacy cron job storage.\n`,
    );
