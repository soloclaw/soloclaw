...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.soloclaw.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["soloclaw backup create", "Create a timestamped backup in the current directory."],
          [
            "soloclaw backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "soloclaw backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "soloclaw backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "soloclaw backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["soloclaw backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "soloclaw backup verify ./2026-03-09T00-00-00.000Z-openclaw-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "soloclaw backup verify ~/Backups/latest.tar.gz --json",
