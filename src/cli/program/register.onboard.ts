...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.soloclaw.ai/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.soloclaw/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. SOLOCLAW_GATEWAY_TOKEN)",
    )
    ...
    .option("--small", "Use small model: gemma4:e2b (8GB RAM)")
    .option("--medium", "Use medium model: qwen3.5:9b (19GB RAM, default)")
    .option("--large", "Use large model: gemma4:26b (24GB RAM)")
