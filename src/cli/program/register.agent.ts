...
${formatHelpExamples([
  ['soloclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['soloclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['soloclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.soloclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.soloclaw.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "soloclaw agents set-identity --identity-file ~/.soloclaw/workspace/IDENTITY.md --agent main",
