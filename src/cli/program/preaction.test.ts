...
beforeEach(() => {
  ...
  originalHideBanner = process.env.SOLOCLAW_HIDE_BANNER;
  ...
  delete process.env.SOLOCLAW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.SOLOCLAW_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("soloclaw");
    program
      .command("agent")
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "soloclaw", "status", "--debug"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "message", "send"],
    });
  ...
  it("loads plugins for local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "soloclaw", "agent", "--local", "--message", "hi"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "soloclaw", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "channels", "add"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@soloclaw/matrix"],
      processArgv: ["node", "soloclaw", "plugins", "install", "@soloclaw/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "plugins", "install", MATRIX_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@soloclaw/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "soloclaw",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "soloclaw", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.SOLOCLAW_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "soloclaw", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "soloclaw", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "soloclaw", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "soloclaw", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "soloclaw", "backup", "create", "--json"],
    });
