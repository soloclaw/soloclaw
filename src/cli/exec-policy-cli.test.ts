...
const mocks = vi.hoisted(() => {
  ...
  let configState: SoloClawConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: SoloClawConfig) => void }) => {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: SoloClawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: SoloClawConfig }>
    >(async () => ({
      path: "/tmp/soloclaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: SoloClawConfig) => void }) => {
