...
describe("tryRouteCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.SOLOCLAW_DISABLE_ROUTE_FIRST;
    originalHideBanner = process.env.SOLOCLAW_HIDE_BANNER;
    delete process.env.SOLOCLAW_DISABLE_ROUTE_FIRST;
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.SOLOCLAW_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  ...
  it("skips config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "soloclaw", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "soloclaw", "status"])).resolves.toBe(true);
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await tryRouteCli(["node", "soloclaw", "agents", "--json"]);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "soloclaw", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "soloclaw", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
  ...
  it("respects SOLOCLAW_HIDE_BANNER for routed commands", async () => {
