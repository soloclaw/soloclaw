...
function buildHooksReport(config: SoloClawConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("soloclaw hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("soloclaw hooks list")}\` to see available hooks.`;
  }
...
export async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as SoloClawConfig;
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.soloclaw.ai/cli/hooks")}\n`,
    );
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `soloclaw plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`soloclaw hooks install` is deprecated; use `soloclaw plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `soloclaw plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`soloclaw hooks update` is deprecated; use `soloclaw plugins update`."),
      );
