...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "soloclaw", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "soloclaw", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "soloclaw", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "soloclaw", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "soloclaw", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "soloclaw", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "soloclaw", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "soloclaw", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "soloclaw", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "soloclaw", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "soloclaw", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "soloclaw", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "soloclaw", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "soloclaw", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "soloclaw", "--help", "status"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("soloclaw memory");
  });
