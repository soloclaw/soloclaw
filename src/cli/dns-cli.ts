...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.soloclaw.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($OPENCLAW_CONFIG_PATH, default ~/.soloclaw/openclaw.json):",
        ),
      );
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active OpenClaw config ($OPENCLAW_CONFIG_PATH, default ~/.soloclaw/openclaw.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
