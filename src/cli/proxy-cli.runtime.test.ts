...
describe("proxy cli runtime", () => {
  const envKeys = [
    "SOLOCLAW_DEBUG_PROXY_DB_PATH",
    "SOLOCLAW_DEBUG_PROXY_BLOB_DIR",
    "SOLOCLAW_DEBUG_PROXY_CERT_DIR",
    "SOLOCLAW_DEBUG_PROXY_SESSION_ID",
  ...
  beforeEach(() => {
    ...
    process.env.SOLOCLAW_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.SOLOCLAW_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.SOLOCLAW_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.SOLOCLAW_DEBUG_PROXY_ENABLED;
    delete process.env.SOLOCLAW_DEBUG_PROXY_SESSION_ID;
  ...
  it("stops the proxy server and ends the session when child spawn fails", async () => {
    ...
    const store = getDebugProxyCaptureStore(
      process.env.SOLOCLAW_DEBUG_PROXY_DB_PATH!,
      process.env.SOLOCLAW_DEBUG_PROXY_BLOB_DIR!,
    );
