...
import type { SoloClawConfig, ConfigFileSnapshot } from "../config/types.soloclaw.js";
...
vi.mock("../infra/soloclaw-root.js", () => ({
  resolveSoloClawPackageRoot: vi.fn(),
  resolveSoloClawPackageRootSync: vi.fn(() => process.cwd()),
}));
...
const { resolveSoloClawPackageRoot } = await import("../infra/soloclaw-root.js");
...
describe("update-cli", () => {
  ...
  const baseConfig = {} as SoloClawConfig;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSoloClawPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveSoloClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          SOLOCLAW_UPDATE_POST_CORE: "1",
          SOLOCLAW_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        SOLOCLAW_UPDATE_POST_CORE: "1",
        SOLOCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SoloClawConfig,
        });
      },
  ...
  it.each([
    ...
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("openclaw-update"));
        await withEnvAsync(
          { SOLOCLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/openclaw-next.tgz" },
          async () => {
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            SOLOCLAW_STATE_DIR: "./state",
            SOLOCLAW_CONFIG_PATH: "./config/soloclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            SOLOCLAW_STATE_DIR: path.resolve("./state"),
            SOLOCLAW_CONFIG_PATH: path.resolve("./config/soloclaw.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            SOLOCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SOLOCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SOLOCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    await withEnvAsync({ SOLOCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
