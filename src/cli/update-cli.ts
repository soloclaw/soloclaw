...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update SoloClaw and inspect update channel status")
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.soloclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.soloclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
