...
import type { ConfigFileSnapshot } from "../config/types.soloclaw.js";
import {
...
const hoisted = vi.hoisted(() => ({
  loadConfigMock: vi.fn<() => SoloClawConfig>(),
...
vi.mock("../commands/doctor/shared/channel-doctor.js", () => ({
  collectChannelDoctorStaleConfigMutations: (cfg: SoloClawConfig) =>
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as SoloClawConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as SoloClawConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  ...
  beforeEach(() => {
    ...
    collectChannelDoctorStaleConfigMutationsMock.mockImplementation(async (cfg: SoloClawConfig) => [
      {
        config: cfg,
        changes: [],
      },
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as SoloClawConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as SoloClawConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as SoloClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
