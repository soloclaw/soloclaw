...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.soloclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SOLOCLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.SOLOCLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.SOLOCLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.SOLOCLAW_CONFIG_PATH)) {
    env.SOLOCLAW_CONFIG_PATH = path.join(stateDir, "soloclaw.json");
  }

  if (profile === "dev" && !env.SOLOCLAW_GATEWAY_PORT?.trim()) {
    env.SOLOCLAW_GATEWAY_PORT = "19001";
  }
}
