...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.soloclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SoloClaw hooks")
    ...
    .option("--hook-url <url>", "SoloClaw hook URL")
    .option("--hook-token <token>", "SoloClaw hook token")
