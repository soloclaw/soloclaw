...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { SOLOCLAW_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("SOLOCLAW_CONTAINER_HINT", "openclaw-demo-container");
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { SOLOCLAW_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("SOLOCLAW_GATEWAY_TOKEN", "env-token");
  ...
  it("resolves config token SecretRefs using service command env before drift checks", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: {
        SOLOCLAW_GATEWAY_TOKEN: "service-token",
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["soloclaw gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["soloclaw gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "soloclaw gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
