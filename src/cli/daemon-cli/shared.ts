...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("soloclaw gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.SOLOCLAW_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.SOLOCLAW_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const hints = buildPlatformServiceStartHints({
    installCommand: formatCliCommand("soloclaw gateway install", env),
    startCommand: formatCliCommand("soloclaw gateway", env),
