...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    SOLOCLAW_STATE_DIR: "/tmp/openclaw-daemon",
    SOLOCLAW_CONFIG_PATH: "/tmp/openclaw-daemon/soloclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.SOLOCLAW_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.SOLOCLAW_CONFIG_PATH ?? `${stateDir}/soloclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SOLOCLAW_STATE_DIR = "/tmp/openclaw-cli";
    process.env.SOLOCLAW_CONFIG_PATH = "/tmp/openclaw-cli/soloclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.SOLOCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.SOLOCLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.SOLOCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
