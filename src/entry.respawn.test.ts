...
import {
  ...
  SOLOCLAW_NODE_EXTRA_CA_CERTS_READY,
  SOLOCLAW_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    const plan = buildCliRespawnPlan({
      argv: ["node", "soloclaw", "gateway", "run"],
      env: {},
      execArgv: [],
      autoNodeExtraCaCerts: "/etc/ssl/certs/ca-certificates.crt",
    });
    ...
    expect(plan?.env[SOLOCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[SOLOCLAW_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "soloclaw", "gateway", "run"],
        env: {
          [SOLOCLAW_NODE_EXTRA_CA_CERTS_READY]: "1",
          [SOLOCLAW_NODE_OPTIONS_READY]: "1",
        },
