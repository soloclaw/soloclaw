...
export type PromptDefaultModelResult = { model?: string; config?: SoloClawConfig };
...
function resolveConfiguredModelRaw(cfg: SoloClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SoloClawConfig): string[] {
...
export function applyModelAllowlist(cfg: SoloClawConfig, models: string[]): SoloClawConfig {
