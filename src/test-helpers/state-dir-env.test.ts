...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    openclaw: process.env.SOLOCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.SOLOCLAW_STATE_DIR).toBe(snapshot.openclaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips SOLOCLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/openclaw-state-dir-test");
    expect(process.env.SOLOCLAW_STATE_DIR).toBe("/tmp/openclaw-state-dir-test");
