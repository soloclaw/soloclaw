...
describe("Dockerfile", () => {
  it("uses shared multi-arch base image refs for all root Node stages", async () => {
    ...
    expect(dockerfile).toContain(
      'ARG SOLOCLAW_NODE_BOOKWORM_IMAGE="node:24-bookworm@sha256:3a09aa6354567619221ef6c45a5051b671f953f0a1924d1f819ffb236e520e6b"',
    );
    expect(dockerfile).toContain(
      'ARG SOLOCLAW_NODE_BOOKWORM_SLIM_IMAGE="node:24-bookworm-slim@sha256:e8e2e91b1378f83c5b2dd15f0247f34110e2fe895f6ca7719dbb780f929368eb"',
    );
    expect(dockerfile).toContain("FROM ${SOLOCLAW_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${SOLOCLAW_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${SOLOCLAW_NODE_BOOKWORM_IMAGE} AS base-default");
    expect(dockerfile).toContain("FROM ${SOLOCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-slim");
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG SOLOCLAW_INSTALL_BROWSER");
  ...
  it("prunes runtime dependencies after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG SOLOCLAW_EXTENSIONS");
    expect(dockerfile).toContain("ARG SOLOCLAW_BUNDLED_PLUGIN_DIR");
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    ...
    expect(dockerfile).toContain(`ARG SOLOCLAW_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bSOLOCLAW_BUNDLED_PLUGINS_DIR\b/m);
  });

  it("normalizes plugin and agent paths permissions in image layers", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    expect(dockerfile).toContain(
      "RUN for dir in /app/${SOLOCLAW_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \\",
    );
    expect(dockerfile).toContain('find "$dir" -type d -exec chmod 755 {} +');
    expect(dockerfile).toContain('find "$dir" -type f -exec chmod 644 {} +');
  });

  it("Docker GPG fingerprint awk uses correct quoting for SOLOCLAW_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
