...
import type { SoloClawPluginService, SoloClawPluginServiceContext } from "./types.js";
...
function createRegistry(services: SoloClawPluginService[]) {
...
function expectServiceContext(
  ctx: SoloClawPluginServiceContext,
...
function expectServiceLogger(ctx: SoloClawPluginServiceContext) {
...
function expectServiceContexts(
  contexts: SoloClawPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: SoloClawPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: SoloClawPluginServiceContext[] = [];
