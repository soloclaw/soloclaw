...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_SOLOCLAW_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("soloclaw/plugin-sdk/core");
    ...
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "soloclaw/plugin-sdk/global-singleton",
    );
    const textRuntimeSdk = await importResolvedPluginSdkSubpath("soloclaw/plugin-sdk/text-runtime");
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("soloclaw/plugin-sdk/plugin-entry");
    ...
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`soloclaw/plugin-sdk/${id}`));
    }
