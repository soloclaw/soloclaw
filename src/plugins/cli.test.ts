...
const mocks = vi.hoisted(() => ({
  ...
  loadSoloClawPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadSoloClawPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadSoloClawPluginCliRegistry(...args),
  loadSoloClawPlugins: (...args: unknown[]) => mocks.loadSoloClawPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: SoloClawConfig;
  autoEnabledConfig: SoloClawConfig;
  ...
  expect(mocks.loadSoloClawPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadSoloClawPluginCliRegistry.mockReset();
    mocks.loadSoloClawPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadSoloClawPlugins.mockReset();
    mocks.loadSoloClawPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as SoloClawConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as SoloClawConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as SoloClawConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadSoloClawPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadSoloClawPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadSoloClawPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("lazy-registers descriptor-backed plugin commands on first invocation", async () => {
    ...
    await registerPluginCliCommands(program, {} as SoloClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as SoloClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as SoloClawConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadSoloClawPlugins).not.toHaveBeenCalled();
  });
});
