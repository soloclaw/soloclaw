...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). SoloClaw uses this to expose
 ...
 *
 * Runs after SoloClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common SoloClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and SoloClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before SoloClaw builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. SoloClaw no
...
/**
 * Provider-owned stream wrapper hook after SoloClaw applies its generic
 ...
 *
 * Runs only after SoloClaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that SoloClaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after SoloClaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, SoloClaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * SoloClaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after SoloClaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before SoloClaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic SoloClaw wrappers.
   *
   ...
   *
   * Called after SoloClaw resolves the raw configured credential but before the
   ...
   * `errorMessage` when SoloClaw should surface a provider-specific hint for
   ...
   * merging. SoloClaw deduplicates by `provider/id`, so plugins only need to
   ...
   * rewrites after SoloClaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before SoloClaw handles
   ...
   *
   * SoloClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `soloclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `soloclaw doctor` should offer.
   *
   ...
   *
   * SoloClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * SoloClaw falls back to that stored profile.
   */
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active SoloClaw session when available. */
  sessionFile?: string;
 ...
 * If you want SoloClaw to keep a plugin command lazy-loaded while still
...
export type SoloClawPluginApi = {
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, SoloClaw can keep
