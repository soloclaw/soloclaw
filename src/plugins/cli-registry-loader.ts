...
export type PluginCliCommandGroupEntry = {
  pluginId: string;
  placeholders: readonly SoloClawPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: SoloClawPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadSoloClawPluginCliRegistry(
...
): Promise<SoloClawPluginCliCommandDescriptor[]> {
  try {
