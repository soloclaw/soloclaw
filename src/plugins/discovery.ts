...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SOLOCLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.SOLOCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @soloclaw/voice-call -> voice-call).
