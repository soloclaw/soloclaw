...
function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "soloclaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.SOLOCLAW_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveSoloClawPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveSoloClawPackageRootSync({ argv1: process.argv[1] }),
