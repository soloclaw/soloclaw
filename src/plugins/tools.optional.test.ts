...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadSoloClawPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { SOLOCLAW_HOME: "/srv/openclaw-home" },
      },
    },
    {
