...
const mocks = vi.hoisted(() => ({
  loadSoloClawPlugins: vi.fn<typeof import("../loader.js").loadSoloClawPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadSoloClawPlugins: (...args: Parameters<typeof mocks.loadSoloClawPlugins>) =>
    mocks.loadSoloClawPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadSoloClawPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadSoloClawPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadSoloClawPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
});
