...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.SOLOCLAW_LAZY_SERVICE_SKIP;
    delete process.env.SOLOCLAW_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.SOLOCLAW_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "SOLOCLAW_LAZY_SERVICE_OVERRIDE",
  ...
  it("surfaces override validation failures", async () => {
    process.env.SOLOCLAW_LAZY_SERVICE_OVERRIDE = "data:text/javascript,boom";
