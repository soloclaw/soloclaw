...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "soloclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "soloclaw.plugin.json");
  const linkedManifest = path.join(rootDir, "soloclaw.plugin.json");
  ...
  return loadPluginManifestRegistry({
    ...
    candidates: [
      createPluginCandidate({
        ...
        packageManifest: {
          install: {
            npmSpec: "@soloclaw/synology-chat",
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "soloclaw.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    ...
    },
    {
      ...
      minHostVersion: ">=2026.3.22",
      env: { SOLOCLAW_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        SOLOCLAW_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        SOLOCLAW_HOME: undefined,
        SOLOCLAW_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
