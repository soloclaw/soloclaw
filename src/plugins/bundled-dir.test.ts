...
const originalBundledDir = process.env.SOLOCLAW_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS;
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@soloclaw/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.SOLOCLAW_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.SOLOCLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
  if (originalDisableBundledPlugins === undefined) {
    delete process.env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.SOLOCLAW_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
