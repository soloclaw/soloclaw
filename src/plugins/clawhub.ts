...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "soloclaw skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this SoloClaw runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires SoloClaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          ...
          normalizeOptionalString(process.env.SOLOCLAW_CLAWHUB_URL) ||
          "https://clawhub.ai",
