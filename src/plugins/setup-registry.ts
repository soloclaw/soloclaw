...
function collectConfiguredPluginEntryIds(config: SoloClawConfig): string[] {
...
function resolveRegister(mod: SoloClawPluginModule): {
...
export function resolvePluginSetupRegistry(params?: {
  ...
  for (const record of manifestRegistry.plugins) {
    ...
    let mod: SoloClawPluginModule;
    try {
      mod = getJiti(setupSource)(setupSource) as SoloClawPluginModule;
    } catch {
      continue;
    }

    const resolved = resolveRegister((mod as { default?: SoloClawPluginModule }).default ?? mod);
