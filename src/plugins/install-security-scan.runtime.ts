...
function buildScanFailureBlockReason(params: { error: string; targetLabel: string }) {
  return `${params.targetLabel} blocked: code safety scan failed (${params.error}). Run "soloclaw security audit --deep" for details.`;
}
...
export async function scanBundleInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Bundle "{target}" has {count} suspicious code pattern(s). Run "soloclaw security audit --deep" for details.`,
...
export async function scanPackageInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Plugin "{target}" has {count} suspicious code pattern(s). Run "soloclaw security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanFileInstallSourceRuntime(
  ...
  const builtinScan = await scanFileTarget({
    ...
    suspiciousMessage: `Plugin file "{target}" has {count} suspicious code pattern(s). Run "soloclaw security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanSkillInstallSourceRuntime(params: {
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    path: params.sourceDir,
    suspiciousMessage:
      'Skill "{target}" has {count} suspicious code pattern(s). Run "soloclaw security audit --deep" for details.',
