...
const discoverSoloClawPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverSoloClawPlugins: (...args: unknown[]) => discoverSoloClawPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverSoloClawPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
  ...
  loadPluginManifestMock.mockImplementation((rootDir: string) =>
    rootDir in manifestIds
      ...
      : {
          ...
          manifestPath: `${rootDir}/soloclaw.plugin.json`,
        },
  );
}
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@soloclaw/${params.pluginId}`,
  };
}
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverSoloClawPluginsMock.mockReset();
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    expect(discoverSoloClawPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverSoloClawPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@soloclaw/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});
