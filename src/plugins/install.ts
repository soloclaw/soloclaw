...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing openclaw.extensions; update the plugin package to include openclaw.extensions (for example ["./dist/index.js"]). See https://docs.soloclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-openclaw-extensions';
...
function ensureSoloClawExtensions(params: { manifest: PackageManifest }):
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_SOLOCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_SOLOCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "soloclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function installPluginFromPackageDir(
  ...
  // Prefer the canonical `id` from soloclaw.plugin.json over the npm package name.
