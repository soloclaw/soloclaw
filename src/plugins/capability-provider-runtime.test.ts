...
const mocks = vi.hoisted(() => ({
  ...
  withBundledPluginAllowlistCompat: vi.fn(
    ({ config, pluginIds }: { config?: SoloClawConfig; pluginIds: string[] }) =>
      ({
        ...config,
        plugins: {
          ...config?.plugins,
...
function expectBundledCompatLoadPath(params: {
  cfg: SoloClawConfig;
  allowlistCompat: SoloClawConfig;
  enablementCompat: {
    plugins: {
      allow?: string[];
...
function createCompatChainConfig() {
  const cfg = { plugins: { allow: ["custom-plugin"] } } as SoloClawConfig;
...
describe("resolvePluginCapabilityProviders", () => {
  ...
  it("keeps active capability providers even when cfg is passed", () => {
    ...
    const providers = resolvePluginCapabilityProviders({
      key: "speechProviders",
      cfg: { messages: { tts: { provider: "edge" } } } as SoloClawConfig,
    });
