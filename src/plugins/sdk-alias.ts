...
function hasTrustedSoloClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasSoloClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("soloclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.soloclaw === "string");
  const hasSoloClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "soloclaw.mjs"));
  return hasCliEntryExport || hasSoloClawBin || hasSoloClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedSoloClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedSoloClawRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedSoloClawRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveSoloClawPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedSoloClawRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["soloclaw/plugin-sdk", "@soloclaw/plugin-sdk"] as const;
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `soloclaw/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "soloclaw/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.soloClawPackageRoot, "extensions"),
    path.join(params.soloClawPackageRoot, "dist", "extensions"),
    path.join(params.soloClawPackageRoot, "dist-runtime", "extensions"),
