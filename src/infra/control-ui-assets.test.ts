...
vi.mock("./soloclaw-root.js", () => ({
  resolveSoloClawPackageRoot: vi.fn(async () => null),
  resolveSoloClawPackageRootSync: vi.fn(() => null),
}));
...
let openclawRoot: typeof import("./soloclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openclawRoot = await import("./soloclaw-root.js");
  });
  ...
  it("uses resolveSoloClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openclaw");
    (
      openclawRoot.resolveSoloClawPackageRoot as unknown as ReturnType<typeof vi.fn>
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "soloclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "soloclaw.mjs"))).resolves.toBe(
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    (
      openclawRoot.resolveSoloClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/SoloClaw.app/Contents/MacOS/SoloClaw");
    const bundledUiDir = abs("fixtures/SoloClaw.app/Contents/Resources/control-ui");
