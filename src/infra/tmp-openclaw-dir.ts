...
export function resolvePreferredOpenClawTmpDir(
  ...
  const existingPreferredState = resolveDirState(POSIX_SOLOCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_SOLOCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_SOLOCLAW_TMP_DIR)) {
      return POSIX_SOLOCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_SOLOCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_SOLOCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_SOLOCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_SOLOCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_SOLOCLAW_TMP_DIR;
