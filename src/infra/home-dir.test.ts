...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers SOLOCLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for SOLOCLAW_HOME",
      env: { SOLOCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when SOLOCLAW_HOME is tilde-only and no fallback home exists",
      env: { SOLOCLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores SOLOCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          SOLOCLAW_HOME: "/srv/openclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { SOLOCLAW_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
      },
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of SOLOCLAW_HOME", () => {
    expect(
