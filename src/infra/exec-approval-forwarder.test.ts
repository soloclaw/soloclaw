...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): SoloClawConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): SoloClawConfig {
  return {
