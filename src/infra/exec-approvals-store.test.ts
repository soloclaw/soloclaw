...
function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".soloclaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".soloclaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".soloclaw", "exec-approvals.sock")),
    );
  });
  ...
  it("refuses to traverse a symlinked parent component in the approvals path", () => {
    ...
    expect(fs.existsSync(path.join(realHome, ".soloclaw"))).toBe(false);
  });
