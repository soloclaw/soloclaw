...
const PRIMARY_PACKAGE_NAME = "soloclaw";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const SOLOCLAW_MAIN_PACKAGE_SPEC = "github:soloclaw/soloclaw#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "SoloClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.SOLOCLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.SOLOCLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return SOLOCLAW_MAIN_PACKAGE_SPEC;
  }
