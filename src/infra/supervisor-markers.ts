const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "SOLOCLAW_LAUNCHD_LABEL"],
  systemd: ["SOLOCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["SOLOCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.SOLOCLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.SOLOCLAW_SERVICE_KIND?.trim();
