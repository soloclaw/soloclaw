...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SOLOCLAW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "SoloClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(SoloClaw\)\s*$/i, "").trim() || normalized;
}
