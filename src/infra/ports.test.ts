...
describe("ports helpers", () => {
  ...
  it("prints an OpenClaw-specific hint when port details look like another SoloClaw instance", async () => {
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js soloclaw gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another SoloClaw instance is already running");
  });
});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("soloclaw");
