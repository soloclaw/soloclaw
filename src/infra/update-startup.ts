...
import { resolveSoloClawPackageRoot } from "./soloclaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: SoloClawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: SoloClawConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("soloclaw update")}`,
      );
