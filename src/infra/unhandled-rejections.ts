...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[soloclaw] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  ...
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[soloclaw] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[soloclaw] FATAL unhandled rejection:", formatUncaughtError(reason));
      exitWithTerminalRestore("fatal unhandled rejection");
      return;
    }

    if (isConfigError(reason)) {
      console.error("[soloclaw] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
    ...
    if (isTransientUnhandledRejectionError(reason)) {
      console.warn(
        "[soloclaw] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[soloclaw] Unhandled promise rejection:", formatUncaughtError(reason));
