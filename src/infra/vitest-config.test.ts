...
describe("resolveLocalVitestMaxWorkers", () => {
  ...
  it("lets SOLOCLAW_VITEST_MAX_WORKERS override the inferred cap", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          SOLOCLAW_VITEST_MAX_WORKERS: "2",
        },
        {
          cpuCount: 10,
          loadAverage1m: 0,
  ...
  it("respects the legacy SOLOCLAW_TEST_WORKERS override too", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          SOLOCLAW_TEST_WORKERS: "3",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0,
...
describe("resolveLocalVitestScheduling", () => {
  ...
  it("allows disabling the system throttle probe explicitly", () => {
    expect(
      resolveLocalVitestScheduling(
        {
          SOLOCLAW_VITEST_DISABLE_SYSTEM_THROTTLE: "1",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0.5,
...
describe("test scripts", () => {
  it("keeps test scripts on the native thread-first configs", () => {
    ...
    expect(pkg.scripts?.["test:serial"]).toBe(
      "SOLOCLAW_TEST_PROJECTS_SERIAL=1 SOLOCLAW_VITEST_MAX_WORKERS=1 node scripts/test-projects.mjs",
    );
