...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "soloclaw.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "soloclaw.plugin.json");
...
function statusCommandSpawn() {
  return [process.execPath, "soloclaw.mjs", "status"];
}
    ...
    async () => {
      await withTempDir({ prefix: "openclaw-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "soloclaw.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "openclaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "soloclaw.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "soloclaw.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
