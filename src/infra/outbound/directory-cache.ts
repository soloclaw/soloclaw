...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SoloClawConfig | null = null;
  ...
  get(key: string, cfg: SoloClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SoloClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: SoloClawConfig): void {
