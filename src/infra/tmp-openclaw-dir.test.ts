...
import { POSIX_SOLOCLAW_TMP_DIR, resolvePreferredOpenClawTmpDir } from "./tmp-openclaw-dir.js";
...
describe("resolvePreferredOpenClawTmpDir", () => {
  it("prefers /tmp/openclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SOLOCLAW_TMP_DIR);
  ...
  it("prefers /tmp/openclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SOLOCLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });
  ...
  it("repairs existing /tmp/openclaw permissions when they are too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_SOLOCLAW_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_SOLOCLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_SOLOCLAW_TMP_DIR, 0o700);
  ...
  it("repairs /tmp/openclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_SOLOCLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_SOLOCLAW_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SOLOCLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_SOLOCLAW_TMP_DIR, 0o700);
