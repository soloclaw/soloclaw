...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SoloClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: SoloClawConfig) {
...
function resolveHeartbeatAgents(cfg: SoloClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SoloClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SoloClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SoloClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
