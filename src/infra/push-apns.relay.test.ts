  ...
  describe("resolveApnsRelayConfigFromEnv", () => {
    it("returns a missing-config error when no relay base URL is configured", () => {
      expect(resolveApnsRelayConfigFromEnv({} as NodeJS.ProcessEnv)).toEqual({
        ok: false,
        error:
          "APNs relay config missing: set gateway.push.apns.relay.baseUrl or SOLOCLAW_APNS_RELAY_BASE_URL",
      });
    });
    ...
    it("lets env overrides win and clamps tiny timeout values", () => {
      const resolved = resolveApnsRelayConfigFromEnv(
        {
          SOLOCLAW_APNS_RELAY_BASE_URL: " https://relay-override.example.com/base/ ",
          SOLOCLAW_APNS_RELAY_TIMEOUT_MS: "999",
    ...
    it("allows loopback http URLs for alternate truthy env values", () => {
      const resolved = resolveApnsRelayConfigFromEnv({
        SOLOCLAW_APNS_RELAY_BASE_URL: "http://[::1]:8787",
        SOLOCLAW_APNS_RELAY_ALLOW_HTTP: "yes",
        SOLOCLAW_APNS_RELAY_TIMEOUT_MS: "nope",
      } as NodeJS.ProcessEnv);
    ...
    it.each([
      {
        ...
        env: { SOLOCLAW_APNS_RELAY_BASE_URL: "ftp://relay.example.com" },
        expected: "unsupported protocol",
      },
      {
        name: "http non-loopback host",
        env: {
          SOLOCLAW_APNS_RELAY_BASE_URL: "http://relay.example.com",
          SOLOCLAW_APNS_RELAY_ALLOW_HTTP: "true",
        },
        expected: "loopback hosts",
      },
      {
        name: "query string",
        env: { SOLOCLAW_APNS_RELAY_BASE_URL: "https://relay.example.com/path?debug=1" },
        expected: "query and fragment are not allowed",
      },
      {
        name: "userinfo",
        env: { SOLOCLAW_APNS_RELAY_BASE_URL: "https://user:pass@relay.example.com/path" },
