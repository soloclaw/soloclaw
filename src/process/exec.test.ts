...
import { SOLOCLAW_CLI_ENV_VALUE } from "../infra/openclaw-exec-env.js";
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        SOLOCLAW_BASE_ENV: "base",
        SOLOCLAW_TO_REMOVE: undefined,
      },
      env: {
        SOLOCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.SOLOCLAW_BASE_ENV).toBe("base");
    expect(resolved.SOLOCLAW_TEST_ENV).toBe("ok");
    expect(resolved.SOLOCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.SOLOCLAW_CLI).toBe(SOLOCLAW_CLI_ENV_VALUE);
  });
