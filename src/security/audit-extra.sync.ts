...
function collectModels(cfg: SoloClawConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function isGatewayRemotelyExposed(cfg: SoloClawConfig): boolean {
...
function listKnownNodeCommands(cfg: SoloClawConfig): Set<string> {
...
function listGroupPolicyOpen(cfg: SoloClawConfig): string[] {
  const out: string[] = [];
...
function listPotentialMultiUserSignals(cfg: SoloClawConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: SoloClawConfig): {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SOLOCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("soloclaw security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SOLOCLAW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const openclawGatewayToken =
    typeof env.SOLOCLAW_GATEWAY_TOKEN === "string" && env.SOLOCLAW_GATEWAY_TOKEN.trim()
      ? env.SOLOCLAW_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectSandboxDockerNoopFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "SoloClaw's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
