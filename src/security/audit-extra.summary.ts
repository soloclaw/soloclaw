...
function summarizeGroupPolicy(cfg: SoloClawConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: SoloClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SoloClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SoloClawConfig): boolean {
...
function isBrowserEnabled(cfg: SoloClawConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: SoloClawConfig): SecurityAuditFinding[] {
